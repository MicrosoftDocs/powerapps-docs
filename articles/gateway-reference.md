...
When a user interacts with an element  connected to an on-premises data source:  
1. The cloud service creates a query, along with the encrypted credentials for the on-premises data source, and sends the query to the queue for the gateway to process.
...
**Question:** Do I need a gateway for data sources, such as SQL Azure, in the cloud?  
...
**Answer:** See [Ports](gateway-reference.md#ports) and hosts that the gateway uses.
...
[Learn more](https://azure.microsoft.com/documentation/articles/service-bus-fundamentals-hybrid-solutions/)
...
The gateway runs as a Windows service; as with any other Windows service, you can start and stop it in multiple ways. To start and stop the gateway from a command prompt.
...
The results should look similar to the following. The difference will be with TcpTestSucceeded. If **TcpTestSucceeded** is not *true*, you may be blocked by a firewall.
...
If you want to be exhaustive, substitute the **ComputerName** and **Port** values with those listed for [ports](gateway-reference.md#ports)
...
Graphic: powerbi-gateway-enterprise-tshoot/event-logs.png
...
Graphic: powerbi-gateway-enterprise-tshoot/fiddler.png
