...
Errors can happen when changing a record in a data source.  There is a wide spectrum of reasons, including network outages, inadequate permissions, edit conflicts, etc.  If there is an error, the **[Patch](function-patch.md)** and other data functions will return *blank*.  You can then use the **Errors** function to obtain the details. 
...
- **Column**.  The [column](working-with-tables.md) name (as a string) of the column that caused the error, if the error can be attributed to a single column.  If not, this column will be *blank*.

- **Message**.  Description of the error (as a string).  This error string can be displayed to the end user, but be aware that it may be generated by the data source and could be long and contain column names that may not have any meaning to the end user. 

- **Error**.  An error kind that can be used to drive the UI.
...
Changing a record in a data source isn't easy.  Despite careful validation with the **[Validate](function-validate.md)** function, and properly gathering   
When a change is made to records in a data source through the **[Patch](function-patch.md)** and other functions, the change is made locally first and then pushed to the data source's source later.   
