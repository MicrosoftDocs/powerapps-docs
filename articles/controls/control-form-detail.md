...
By default, cards are placed in a single column for phone apps and three columns for table apps.  You can adjust the number of columns and if should cards snap to columns when authoring.  These settings are not exposed as properties as they are only used at authoring time to set the **X**, **Y**, and **Width** properties of the cards.
