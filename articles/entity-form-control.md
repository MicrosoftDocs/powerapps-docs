...
8. (*Optional*) View the **DefaultList** field group of the **Sales order** entity:
	...
	In the **Common Data Service**, you can [modify field groups](field-groups.md) for custom entities (but not [standard entities](learning-common-data-service-entities.md)) to change the fields that appear on the corresponding forms that the **Entity form** control displays. Best of all, any change to the field group is automatically reflected in all the apps that use an **Entity form** control to display the corresponding form.
...
4. Set the **OnSelect** property of the back-arrow icon to the [**Back**](function-navigate.md) function.  
...
The **OnFieldSelect** property of the **SalesOrderListForm** is set to an [**If**](function-if.md) function, which determines whether the user clicks or taps the **Sales order ID** field: **SalesOrderListForm.SelectedField.SalesOrderId = true**.  
...
Therefore, when the user clicks or taps a sales order ID field, the [**If**](function-if.md) function evaluates to true, and the [**Navigate**](function-navigate.md) function is called with the corresponding context, opening the details screen.  
