...
**Card** controls are the building blocks of the **Edit form** and **Display form** controls. The form represents the entire record, and each card represents a single field of that record.
...
## Customize a card ##
Cards comprise other controls. The user enters data in a standard **Text input** control that you add from the **Insert** tab.  
...
As another example, show an image in an **Image** control instead of the image's URL in a **Text-input** control, as this graphic shows
...
3. In the formula bar, set the **Image** property of this control to **TextBox.Text**, where **TextBox** is the name of the **Input text** control that holds the URL:

	**Tip**: Press the Alt key to show the name of each control.

	![](./media/working-with-cards/customize-image3.png)

	And now we can see the images and edit their URLs. Note that we could have used **Parent.Default** as the **Image** property, but it wouldn't have updated as the user entered a new URL.

4. We can do the same thing on the second screen of this app, where we use a **Display form** control to display the details of a record.  Here we may want to hide the text box (set the **Visible** property of the text-box control, not the card, to *false*) because the user won't edit the URL on that screen:
...
In the **Advanced** view, the large banner at the top indicates that the properties of this card are locked. A lock icon also appears next to the **DataField**, **Default**, and **DisplayName** properties. The **Options** pane created these formulas, and the lock prevents accidental changes to these properties.  
...
Let's modify the **DisplayName** to put a space between **Asset** and **ID**. By making this change, we're altering what was generated for us.  In the **Options** pane, this card has a different label:
...
|--------------|--------------|-------------|
| **TextRequiredStar** | **Text box** control | Displays a star, which is commonly used on data-entry forms to indicate that a field is required. |
| **TextFieldDisplayName** | **Text box** control | Displays the user-friendly name of this field. This name can differ from what is in the data source's schema. |
| **InputText** | **Input text** control | Displays the initial value of the field and allows the user to change that value. |
| **TextErrorMessage** | **Text box** control | Displays a user-friendly error message to the user if a problem occurs with validation. Also ensures that the field has a value if one is required. |
...
| **TextFieldDisplayName.Text** | **Parent.DisplayName** | The text-box control shows the user-friendly name, which you or the data source's metadata provides, and which is set on the card's **DisplayName** property. |
...
| **DataCard.Update** | **InputText.Text** | The value to validate and push back into the data source when **SubmitForm** runs. |
