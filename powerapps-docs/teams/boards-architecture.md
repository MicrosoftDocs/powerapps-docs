...
![](media/boards-architecture/architecture-model.png)
...
![](media/boards-architecture/data-model.png)
...
| colDeleteBoardCategories          | Collection of the board categories which are to be removed                           | OnSelect Property of the button save in settings screen  |
    ...
    **gblAppStyles** is not blank (which means the styling variable has been
    ...
    - If the latest Boards User Settings record for the current user (stored in
      **gblRecordUserSettings**) has a value of Yes for ‘Display Splash (Power
      Apps)’ column, then this is set to true, if not, then false.
...
| **Collection name**               | **Description**                                              |
| --------------------------------- | ------------------------------------------------------------ |
| colBoards                         | Collection of Boards which are shown for the users.          |
...
| ------------------------------- | ------------------------------------------------------------ |
| gblAppStyles                    | Global variable to store styling properties for all controls (set on the OnVisible of the Loading Screen). |
| locBlockUserInput               | Local variable to setup the user input to false and update the dialog user preference. |
| locNavFromAbout                 | Local variable to setup the Boolean value as true while navigating from boards screen to About Screen |
...
2.  Don’t show this again once you check this the user settings will be updated
...
| locSelectedBoard                | Global variable to store the users language.                 |
| gblThemeDark                    | Global variable to store if Teams is running in dark mode.   |
| gblThemeHiCo                    | Global variable to store if Teams is running in contrast mode. |
...
The galSortPreferences shows the sort preferences where Users can select the sort preference and the board categories to make them visible or not visible in the boards screen.
...
| locCoverImageSelected        | Local variable to store the image cover selected.            |
...
| Variable name                | Description                                                  |
| ---------------------------- | ------------------------------------------------------------ |
| gblAppStyles                 | Gobal variable to store styling properties for all controls (set on the OnVisible of the loading screen). |
| locScreenReaderAnnouncedText | Local variable to start the announcement of screen reader.   |
| gblThemeDark                 | Global variable to store if Teams is running in dark mode.   |
| locDefaultBoardCoverColor    | Local variable to set the first board cover color.           |
...
| locSelectedBoardColor        | Local variable to store the color of board which is selected. |
...
1.  The warning edit board screen will show once the user selects delete board item in the edit board screen. The user has options to cancel the
    warning which will show the edit board screen or delete the board
...
1.  The Edit board screen will show once the user selects on the selects  a board and click on edit button in the boards item screen user has the
...
2.  Categories shown in the drop down are coming from the board categories which are set up by the admin. the cover icons, colors and cover image can be selected from these galleries galCoverIcons_AddBoard, galCoverColors_AddBoard, galCovers_AddBoard.
...
1.  The board items screen has options to add new board items and can view the existing the board items which are added by the users. Current user who created the board item will have the option to show the edit board item if not the edit option is disabled, and we can search for board item by typing the text in the find an item space the galBoardItems_Nested will show the items which are related to the text by name of the item or the description.
...
The following collection are used when a board item is added.
...
1.  The warning edit board screen will show once the user selects delete board item in the edit board item screen. The user has options to cancel the
    warning which will show the edit board item screen. 
...
| colDeletedBoardCategories | Collection of the board categories which are to be removed. |
...
| locGalleryDefaultCategory | Local variable to store the gallery of categories which are available in the galCategories. |
