---
title: Understand Get connected sample app architecture | Microsoft Docs
...
|**Connection Profile Skill**|Connection Profile Skill stores skill records that are available to users to select from when users add their skills.   Details such as Name, Email, Skill, skills type to which it is associated to  and the date when the skill was created on, modified on are stored in the Connection profile skill table.|
...
| colMySkillsGuid       | collection of my skills which has unique guid for enhancements in the future. | OnVisible Property of the Profile Screen      |
| colMyExpertSkills     | collection of skills which can help others with              | OnVisible Property of the Profile Screen      |
...
4.  The user’s Teams theme/mode is checked to see if the theme is default, dark  or high contrast. The **gblThemeDark** and **gblThemeHiCo** variables are  set accordingly.
...
| locPeerMatchSuggestion        | To check if the colPeerMatchPool is not blank                |
...
1.  If **gblAppStyles** is not blank (which means the styling variable has been
    ...
    **gblFirstRun**: If the current run is the first run of the app as indicated
    by **gblFirstRun**, this is set to true, if not, then false.

    If the latest Get Connected User Settings record for the current user
    (stored in **gblRecordUserSettings**) has a value of Yes for ‘Display Splash
    (Power Apps)’ column, then this is set to true, if not, then false.
...
| gblCurrUserEmail                | Global variable to store the current user email address      |
| gblRecordUserSettings           | Global variable to store the user settings                   |
| locNavigationTimer              | If the navigation timer has been set to true after the time interval the the app settings and sizes will be updated. |
| gblAppSetting_inputMobileOnWeb  | Global variable to scale fonts for mobile-oriented apps, running in desktop. |
| gblAppSetting_inputScaleFontsBy | Global variable for scaling all fonts by a fixed amount.     |
| gblThemeDark                    | Global variable to store if Teams is running in dark mode.   |
| gblThemeHiCo                    | Global variable to store if Teams is running in contrast mode. |
...
2.  Don’t show this again once you check this the user settings will be updated to not show this pop up and if the user doesn’t select the check box and select **Got it** then this pop up will be shown each time.
...
| colMyExpertSkills     | Collection of skills which can help others with.             |
| colMySkillsGUID       | Collection of my skills which has unique guid for enhancements in the future. |
...
2.  The questionnaire that list questions.
...
| locPeerMatchSuggestion        | Local variable used  to check if the colPeerMatchPool is not blank. |
...
1.  The Home Screen shows the connect with someone new, a peer and an expert. The connections for new random match will be fetched from
    colRandomMatchPool, connections for peers will be fetched from
    colPeerMatchPool.
    
2.  Find a connection through skills is fetched from **galskilltags** and Connection Skill Tags table
    
3.  Update your profile like intro, LinkedIn URL, user settings and skills.
...
| Variable name                      | Description                                                  |
| ---------------------------------- | ------------------------------------------------------------ |
| ggblAppStyles                      | Global variable to store styling properties for all controls. |
| gblAppColors                       | Global variable to store the app design colors.              |
| gblThemeDark                       | Global variable to store if Teams is running in dark mode.   |
| locRandomMatchSuggestion           | Local variable to show the connection by randomly shuffling the userid. |
| gblThemeHiCo                       | Global variable to store if Teams is running in contrast mode. |
| gblCurrUserEmail                   | Global variable to store the current user email address.     |
| locShowInvitePeopleDialog          | Local variable to show the invite to the app if the random pool collection less than three connections. |
| locAreRandomMatchSuggestionsEnough | Local variable to check if the count of random match pool are greater than three. |
| locRandomMatchSuggestionEmail      | Local variable to show the connection by randomly shuffling the emails. |

#### New connection execution details

1.  All the connections will be stored in random match pool collection and will
    be shuffled randomly to show the connections to the user. The user can chat with the new connections or schedule a meeting to know more about the connection.
...
1.  To Invite the peers to get connected if the peers count are less than three
    then we can invite more people through posting into the teams.

2.  If the peer count is more than what is required the user can chat or
    schedule a meeting with a peer and connect with them.
...
1.  The Skills match screen will show the person who has the skills to connect
    with from galskillmatchresults gallery
...
| galSelectedExpertMatchSkills      | Gallery to show the selected expert match based on skills.   |

#### Expert connection execution details

1.  The Expert Match screen will show all the skills which are available and
    users can find the skills they are looking for and connect with the expert
    based on skills in gallery selected expert match skills.
...
2.  Selecting the links the websites will open to the Microsoft docs.
