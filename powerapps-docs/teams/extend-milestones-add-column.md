...
In this topic we will learn how to add a new column/field to a work item. For our example, we will add a new field to capture the effort required for a particular work item. We will add this field to the **New** and **Edit** Work Item screens for both desktop and mobile.

## Prerequisites

To complete this lesson, we would need the ability to login into Microsoft Teams which will be available as part of select Microsoft 365 subscriptions and also have the Milestones app installed in a Teams channel. If you don’t have it installed you can install it from https://aka.ms/teamsmilestones.

## Open Power Apps

To start we will open Power Apps in Microsoft Teams.

1.  The Power Apps editor in the Teams app can be popped out as shown below

![Pop out Power Apps app](media/extend-milestones-add-column/pop-out-app.png "Pop out Power Apps app")

2.  To pop out the power apps editor while using the Team web app, right click on the Build tab on top and select the Open in a new tab option to open the editor in a separate tab

## Customizing the Milestones App

To add the column to the app, we will add the field to the underlying table and then add it to the app screens.
...
6.  Select the **data** button from the left menu.
...
10.  The new column gets added to the table

### Adding the new field to the Add/Edit Work Item page

The following changes need to be made to add the Effort field/column on the Add/Edit Work Item page

1.  Copy and paste the Category label on the screen. We will do this so that the styling of the component is consistent with the other controls in the app.
...
2.  Set Text property = “**Effort”**

3.  Set X property = **lblAddWorkItemAssignedTo.X**

4.  Set Y property =**cmbAddWorkItemCategory.Y+cmbAddWorkItemCategory.Height+20**
    
5.  Select Description label

6.  Update X property = **lblAddWorkItemMilestone.X**

7.  Update Y property = **cmbAddWorkItemCategory.Y+cmbAddWorkItemCategory.Height+20**
    
8.  Select the Description text box

9.  Update X property = **lblAddWorkItemMilestone.X**
...
20.  Select the Done button and update the formula and add the following to both the Patch functions **Effort: cmbAddWorkItemEffort.Selected.Value,** right after the say the description formula **Description: txtAddWorkItemDesc.Text,**

![Update description formula](media/extend-milestones-add-column/description-field.png "Update description formula")

21.  The screen will appear as shown below with the newly added column
...
Now that we have added the field, we will add it to the mobile screen as well.
...
4.  We will add a new label with text Effort and a new combo box to capture the effort value, then do the formatting of the screen and finally add the field value to the Patch function on the OnSelect property of the Done button
    
5.  Copy and paste the Status Label

6.  Rename the newly added label to **lblMobileWorkItemDetailEffortLabel**

7.  Set Text property = “**Effort”**

> NOTE
> We are not accounting for Localization in this document hence keeping the text property set to Effort. Ideally, if the app is localized, we will have to update the Localization document to include tags in all the languages for the word **Effort** and then follow all the steps from the Localization document to localize this newly added field. See add localization to template apps (insert link) for more details.
...
14.  Select **shpMobileWorkItemDetailSeparator_7** and update the Height property to **shpMobileWorkItemDetailSeparator\_8.Y +
    shpMobileWorkItemSeparator.Height - Self.Y**
...
26. **Set Text property = locMobileSelectedWorkItem.Effort**
...
1.  Similar to the above scenario, we need to add the Effort field to the New Work Item details screen as well
...
> NOTE 
> We are not accounting for Localization in this document hence keeping the text property set to Effort. Ideally, if the app is localized, we will have to update the Localization document to include tags in all the languages for the word **Effort** and then follow all the steps from the Localization document to localize this newly added field.
...
We have added the new field in the data table and also added it to be displayed on the desktop and mobile screens for work items.
...
> NOTE
> The work items gallery is engineered to be responsive. The header of the gallery uses buttons outside of the gallery. When using buttons outside of a gallery, be aware that the buttons won’t scroll horizontally with the gallery, so you want to limit the number of fields to what will fit in one screen wide without scrolling. It is recommended that you replace one of the exisitng fields rather than adding additional fields to avoid impacting responsiveness or requiring scrolling gallaries.

1.  Select **galWorkItems** from the tree view and expand it.

2.  Identify the column that you want to replace with your new field. For example, if you want to replace the target with your new tracker field, you would selet lblWorkItemETA control.
...
2.  Open the app in the Team and perform a test on the add new work item screen.
