...
2. on the **Email** tab and select **Show email as a conversation on Timeline**. For more information on personal settings, see [Set personal options](./set-personal-options.md#email-tab-options). Once enabled, you can open any form that has a timeline and your emails will be grouped into conversation threads with the latest email at the top.
...
> [!NOTE]
> Recurring appointments are not supported on the [Dynamics 365 App for Outlook](/dynamics365/outlook-app/overview), [Dynamics 365 for phones app](/dynamics365/mobile-app/user-guide-mobile-app), and when you run the model-driven apps web client on your mobile phone web browser.
...
- In the **Timeline** section, select **Open Filter Pane** and select how you want to filter the activities.
 ...
 > When the timeline is displayed on more than one column, the filter pane is displayed as a column alongside the timeline rows. To learn more, see [Filter pane appears in two column mode](../maker/model-driven-apps/faqs-timeline-control.md#why-do-my-agents-see-the-filter-pane-even-when-the-expand-filter-pane-by-default-check-box-is-cleared). 
