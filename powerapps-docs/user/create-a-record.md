...
The **New** command or **Quick create** makes it fast and easy to enter almost any type of information into the system. The command is on the navigation bar whenever you need to enter new information into the system. You can also save a row and create a new one directly from the **Quick create** form.

> [!NOTE]
> The **Quick create** option is only available for rows that are enabled by your admin.
    
1. On the navigation bar, select the **plus (+) icon** ![Create a row button.](media/create-record-button.png "Create a row button"), and then select the item you want.  
...
3. Fill in the columns and then on the command bar, select **Save** or **Save & Close**. The **Save** option is also available in the lower-right corner.

    > [!div class="mx-imgBorder"] 
    > ![Save option on a row.](media/saveoptionalwaysvisible.png "Save option on a row")

   > [!NOTE]
   > If you have unsaved changes and try to go to another row, an **Unsaved changes** dialog box will pop-up. If you select **Save and continue**, the app attempts to save your changes and open the page you wanted to go to. If you choose to save and continue and there is an error on a column, the dialog will close and you will remain on the page to fix the error before you can navigate away.
