...
|`node` or `npm` isn't found, or `/create-mobile-app` fails at prerequisite checks| - Install Node.js 24 LTS and npm 10 or newer. <br> - Restart your terminal and coding agent. <br> - Confirm the installation with `node --version` and `npm --version`.
