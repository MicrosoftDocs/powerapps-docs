...
1. Select an offline profile. You have two options:
    ...
    - Use an autogenerated profile. This setting automatically downloads all the tables used in your app. Autogenerated profiles are a starting point to help makers build an offline profile for initial validation of offline data. These profiles can be resource intensive because they don't compute the optimal filters for each table or assess the amount of data the app must download offline. It's important that app makers review and refine the profile based on their use case.

    - [Create a custom, mobile offline profile](#create-a-custom-mobile-offline-profile-with-admin-rights), select the **Refresh** icon to see your new profile in the list, and then select it. Custom offline profiles let makers add tables and filters during app development by considering their needs and best practices. Custom offline profiles offer flexibility, control, and can be refined to enhance the end user experience by downloading relevant data, improving sync times, and boosting query performance.
