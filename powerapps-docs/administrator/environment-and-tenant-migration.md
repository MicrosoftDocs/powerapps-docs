...
| CDS data rows |No |Export rows from CDS entities is not current supported, so the only option today to manually [export and import](../canvas-apps/data-platform-export-data.md) data after your CDS schema changes have been applied in a new environment. |
