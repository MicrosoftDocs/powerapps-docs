...
In case of a warning or error, you can further drill down by clicking through the executions on the **Execution history** tab.
...
We highly encourage our customers and partners to subscribe to email-based notifications, wherein you receive email alerts on project executions that completed with either warnings or errors. For each project, on the **Scheduling** tab, you will be able to select email-based notifications and provide multiple email addresses (including group addresses), separated by commas.
...
1. **Completed**
   ...
   If all records were upserted ("upsert" or "update insert" is a logic to either update the record, if it already exists, or to insert a new record) successfully, the project status is marked **Completed**. 
...
2. **Warning**
   ...
   If some records were upserted successfully, while some failed or errored out, the project is marked with a **Warning** status.
