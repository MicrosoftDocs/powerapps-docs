...
PowerApps offers users, businesses, independent software vendors (ISVs), and systems integrators (SIs) a powerful platform for building line-of-business apps. **Common Data Service** is the underlying data platform for PowerApps that contains the core functionality of [Dynamics 365 for Customer Engagement platform](/dynamics365/customer-engagement/developer/developer-guide) such as server-side logic (plug-ins and workflows), business process flows, sophisticated security model and support for extending the platform capabilities by developers.
