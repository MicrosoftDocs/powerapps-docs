...
We are in the process of releasing changes that will enable the use of reflection within custom workflow activities. However, in the sandbox environment where this code will run any calls using [Type.GetType Method](/dotnet/api/system.type.gettype) must use the fully qualified name of the assembly. In addition to being unecessary, this code doesn't use the fully qualified name and this is why it throws an error.
