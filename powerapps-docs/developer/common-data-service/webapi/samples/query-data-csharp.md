...
First go to [Web API Query Data Sample (C#)](https://github.com/Microsoft/PowerApps-Samples/tree/master/cds/webapi/C%23), clone or download the sample file, and extract its contents into a local folder. This folder should contain the following files:  
...
|SampleHelper.cs|Contains the helper code to assist in performing common tasks, such as application configuration, authentication and `HTTP` response error handling. | 
