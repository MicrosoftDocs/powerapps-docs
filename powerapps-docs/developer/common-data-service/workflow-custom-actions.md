...
Both of these capabilities allow for creating new messages that can be called from web services. Workflow Custom Actions can also be called by workflows.
...
- With the Web API, the new messages created with Workflow Custom Actions are always actions. For information about calling actions using the Web API, see [Use Web API actions](webapi/use-web-api-actions.md).
