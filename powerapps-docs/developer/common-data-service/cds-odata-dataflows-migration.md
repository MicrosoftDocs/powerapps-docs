...
1. If you are migrating data, make sure the entities are already defined in the target environment. Ideally both environments should have the same entities defined. 
1. When importing relationships, multiple dataflows are needed
    - One (Parent) to many (children) entities require separate dataflows. Configure the parent dataflows to run before any child entities, since the data in the parent needs to be loaded first to correctly map to the fields in the corresponding child entities.

[!Note]
Common Data Service 'instance' and 'environment' are used interchangeably in this article. Each environment in the Power Platform can have 0 or 1 CDS instance(s). Read documentation on environments to learn more on this topic.

### Get the OData endpoint 
Since Common Data Service provides an OData endpoint that does not require any additional configuration to connect with Azure AD, it is the quickest way to connect dataflows to data in the source instance.

This article will walk through how to use the dataflows interface specifically with the OData connector. The generic steps for connecting to all data sources supported by dataflows are in the [documentation](https://docs.microsoft.com/en-us/powerapps/maker/common-data-service/create-and-use-dataflows).
...
1. In the top ribbon, click the chevron next to Settings, then navigate to Customizations > Customizations
...
![copy the service root URL in the developer resources](./media/odata-url.png)
...
1. Select **Create**.
![Prompt for a new dataflow](./media/new-dataflow.png)
1. Select the OData connector, and make sure the field values are correct:
    ...
    ![](./media/connection-settings.png)
                 
    [!Note] 
    You would need to disable pop-up and cookies blocker in your browser in order to configure the AAD authentication. This is orthogonal to the fact that you are using the CDS OData endpoint or any other OAuth-based authentication data source. 
...
First, select the entities that will be transferred. Here you can browse all entities exposed by the Common Data Service OData endpoint you provided and preview some of the data in each entity. 
![Power query navigator](./media/pq-choose-data.png)

1. Select one or multiple entities as needed, then select **Next**.

    [!Note]
    When importing relationships, remember that the parent (independent) entity dataflows need to be imported before the child (dependent) ones.
...
[!Tip]
If you forgot to add an entity, you can choose 'more tables' in the **Get data** ribbon option for the same OData connector.

![](./media/get-data-option.png)

### Configure destination environment settings

Note: If entity relationships exist, import the parent entity in its own dataflow first. The data for the child dataflow will require data to be in the parent entity for it to correctly map.

#### Map entities. 
For each entity chosen, select the behavior for importing that entity in these settings. 

![](./media/map-entities.png)
...
- There is an option to 'Do not load', but it is best not to include entities in the dataflow that are not being loaded. You can select **Back** from this menu to return to the power query menu and 
...
You can manually initiate a dataflow in three dots menu (...) in the dataflows list.

![Refresh manually](./media/click-refresh.png) 
