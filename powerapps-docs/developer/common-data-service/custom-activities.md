...
In Common Data Service, you can create custom activities to support the communication needs of a business such as instant messaging (IM) and Short Message Service (SMS). To create a custom activity in Common Data Service, create a custom table, and specify it as an activity table using the <xref:Microsoft.Xrm.Sdk.Metadata.EntityMetadata>.<xref:Microsoft.Xrm.Sdk.Metadata.EntityMetadata.IsActivity> property.  
  
 However, unlike other custom tables, you can’t specify a primary attribute for a custom activity because, by default, each custom activity must have a primary attribute named ”Subject”.  
  
 When you create a custom activity table, all the properties and privileges of the `activitypointer` table are inherited for the custom activity. Further, all the activity party types become available for the custom activity, and as a result the corresponding properties are also inherited.  
...
## Privileges and access rights 
 ...
 You require the same set of Common Data Service privileges and access rights to work with custom activities as those required to work with custom tables. For more information about custom tables, see [Customize table metadata](customize-entity-metadata.md).  
  
## Creating a custom activity  
 To create a custom activity table, set the values of the properties listed in the following table.  
...
### See also  
 ...
 [Sample: Create a custom table](/dynamics365/customer-engagement/developer/sample-create-custom-activity)   
 [Sample: Create and update table metadata](/dynamics365/customer-engagement/developer/org-service/sample-create-update-entity-metadata)
