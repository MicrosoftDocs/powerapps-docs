...
> [!NOTE]
> To ensure best performance, each query request can return a maximum of 5000 table rows. To return larger result sets you must request additional pages.

### Null column values are not returned

When an column contains a null value, or if the column was not included in the FetchXml columns or the <xref:Microsoft.Xrm.Sdk.Query.ColumnSet>, the <xref:Microsoft.Xrm.Sdk.Entity>.<xref:Microsoft.Xrm.Sdk.Entity.Attributes> collection will not include the column. There will be neither a key to access it or a value to return. The absence of the column indicates that it is null. When using the early bound style, the generated table class properties will manage this and return a null value.
...
1. For an column that could be null, use the <xref:Microsoft.Xrm.Sdk.Entity>.<xref:Microsoft.Xrm.Sdk.Entity.Contains(System.String)> method to check whether the column is null before attempting to access it with an indexer. For example:
...
> [!IMPORTANT]
> When retrieving table rows you should only request the columns values you need by setting the specific columns using `attribute` elements rather than using the `all-attributes` element to return all columns.
...
> [!NOTE]
> Attributes which contain null values are not returned in the query `Attributes` or `FormattedValues` collections. If an column may contain a null value you should check using the <xref:Microsoft.Xrm.Sdk.Entity.Contains*> method before attempting to access the value.
