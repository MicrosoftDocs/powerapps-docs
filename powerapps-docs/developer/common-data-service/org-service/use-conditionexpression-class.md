...
description: "Read how you can use the ConditionExpression class to compare an column to a value or set of values by using an operator, such as &quot;equal to&quot; or &quot;greater than&quot;" # 115-145 characters including spaces. This abstract displays in the search result.
ms.custom: ""
ms.date: 08/03/2020
ms.reviewer: "pehecke"
...
```  
 ...
 When you create a condition that compares an column value to an enumeration, such as a state code, you must use the `ToString` method to convert the value to a string.  
...
This code creates a condition to return only rows where the first and last names are the same, while

```csharp
new ConditionExpression("firstname", ConditionOperator.Equal, false, "John");
```

creates a condition to return only rows where the first name is John.
