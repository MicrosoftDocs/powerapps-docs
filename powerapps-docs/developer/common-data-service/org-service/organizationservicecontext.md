...
After you create the organization service context object, you can begin to track create, modify, or delete tables. 
  
The organization service context must track any table or relationship that you want to submit to Common Data Service. For example, you could retrieve a row with a LINQ query and the context would track that table or you could use the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext>.<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.Attach(Microsoft.Xrm.Sdk.Entity)> method to cause the context to begin tracking the table. You can work with data in a client application and create new tables, create related tables, and modify existing tables, but you must call the `SaveChanges` method on tracked tables to commit changes to Common Data Service.  
  
<a name="track_changes"></a>

## Track changes
 
To determine how an table is tracked by the context, you can check the <xref:Microsoft.Xrm.Sdk.Entity.EntityState> property on the table instance. You must notify the organization service context to track an table from Common Data Service by calling various methods or by using a LINQ query. All tables returned from a LINQ query are tracked by the service context.  
...
|Method|Use|  
|------------|---------|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AddObject(Microsoft.Xrm.Sdk.Entity)>|Adds an table to the set of tables the organization service context is tracking. The status of the table in the context is set to `Created`. If the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.SaveChanges> method is called, this row will be created or added to the server.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.Attach(Microsoft.Xrm.Sdk.Entity)>|Adds an table to the set of tables the organization service context is tracking. The status of the table in the context is set to `Unchanged`. If the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.SaveChanges> method is called, this table will not be sent to the server unless its status changes.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.CreateQuery(System.String)>|Adds the results of a query to the set of tables the organization service context is tracking.|  
  
<a name="track_related"></a>

## Track related objects

In Common Data Service, the organization service context lets you create and update relationships between tables. The navigation properties generated by the CrmSvcUtil.exe tool and located in the early-bound classes let you access and change related table properties and relationships. The organization service context must be tracking the related table for the related table to be available to be updated on the server.  
  
Use the following methods in <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext> to work with related tables and to add the table to the service context:  
  
|Method|Use|  
|------------|---------|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AddRelatedObject(Microsoft.Xrm.Sdk.Entity,Microsoft.Xrm.Sdk.Relationship,Microsoft.Xrm.Sdk.Entity)>|Adds the target to the context. Calls the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.Attach(Microsoft.Xrm.Sdk.Entity)> method on the target table and then calls the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AddLink(Microsoft.Xrm.Sdk.Entity,Microsoft.Xrm.Sdk.Relationship,Microsoft.Xrm.Sdk.Entity)> method between the source table and the target (related) table.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AttachLink(Microsoft.Xrm.Sdk.Entity,Microsoft.Xrm.Sdk.Relationship,Microsoft.Xrm.Sdk.Entity)>|Adds the related table to the context for tracking. The status of the table in the context is set to `Unchanged`.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AddLink(Microsoft.Xrm.Sdk.Entity,Microsoft.Xrm.Sdk.Relationship,Microsoft.Xrm.Sdk.Entity)>|Creates a relationship between the source and target tables. Adds the target to the context. The status of the target table in the context is set to `Created`.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.LoadProperty(Microsoft.Xrm.Sdk.Entity,System.String)>|Loads the related table set for the specified relationship. Gives access to related tables by using the navigation property. Call the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AddObject(Microsoft.Xrm.Sdk.Entity)> method on the related table after you have accessed the table by using a navigation property on the parent table.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.UpdateObject(Microsoft.Xrm.Sdk.Entity)>|Changes the state of the specified table in the `OrganizationServiceContext` to Modified.|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.DeleteObject(Microsoft.Xrm.Sdk.Entity)>|Changes the state of the specified table to be deleted in the `OrganizationServiceContext`.|  
  
<a name="BKMK_LoadRelatedEntities"></a>

### Load related tables using navigation properties

Related tables for tables you have retrieved using LINQ will be null until you use <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.LoadProperty(Microsoft.Xrm.Sdk.Entity,Microsoft.Xrm.Sdk.Relationship)> to retrieve them. The following code sample shows how to access Task rows associated with a specific Contact row.  
...
The organization service context holds a graph of the tables it is tracking. The order in which the organization service context processes table changes and submits them to the server is important. Updates to primary table are processed, and then related tables are processed. If a value is set on the primary table by the related table, that value is used when updating data on the server.  
  
If an error occurs when table information is being saved, a new exception type that contains the <xref:Microsoft.Xrm.Sdk.SaveChangesResult> is thrown by the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext>.<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.SaveChanges> method, regardless of the value of the <xref:Microsoft.Xrm.Sdk.Client.SaveChangesOptions> parameter that is passed into the method.  
...
|Method|Description|  
|------------|-----------------|  
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.OnBeginEntityTracking(Microsoft.Xrm.Sdk.Entity)>|Called after an table is attached to the `OrganizationServiceContext`.|  
...
|<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.OnEndEntityTracking(Microsoft.Xrm.Sdk.Entity)>|Called after an table is detached from the `OrganizationServiceContext`.|  
...
When working with early-bound table classes, you use the table name and column schema name to specify an table or column to work with. Attribute schema names are defined in <xref:Microsoft.Xrm.Sdk.Metadata.EntityMetadata>.<xref:Microsoft.Xrm.Sdk.Metadata.EntityMetadata.SchemaName> and <xref:Microsoft.Xrm.Sdk.Metadata.AttributeMetadata>.<xref:Microsoft.Xrm.Sdk.Metadata.AttributeMetadata.SchemaName>, or you can use the class and property names shown in the code-generated file.The following sample shows how to assign a value to the email column of a new contact instance.  
...
## Create a new table row
 ...
 When you want to insert data into Common Data Service by using the table data model, you must create an instance of an table type and add the object to an organization service context. The organization service context must be tracking the object before it can save the object to Common Data Service.  
  
 When creating a new table row, you add the object to the organization service context by using the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.AddObject(Microsoft.Xrm.Sdk.Entity)>. method.  
  
 The following sample shows how to instantiate and save a new contact row by using the table data model. It also demonstrates how to access a custom column.  
...
Common Data Service tracks changes to objects that are attached to the organization service context. To modify an existing table row, you must first add the object to the context. To add an object to the context, you must first retrieve the table row from Common Data Service and then add the object to the context by using the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.Attach(Microsoft.Xrm.Sdk.Entity)> method. Once the object is being tracked by the context, you can update the row by setting the table's columns.  
...
```  
 ...
 The following sample shows how to delete an column value.  
  
```csharp  
Account.EMailAddress1 = null;  
```  
  
 There are two partial methods named `OnPropertyChanging` and `OnPropertyChanged` for each table. These methods are called in the property setter. You can extend these methods by using partial classes to insert custom business logic.  
  
<a name="delete"></a>   

## Delete an table row   

To delete an table row, the organization service context must be tracking the object. Once the object is on the context, you can use the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.DeleteObject(Microsoft.Xrm.Sdk.Entity)> method to mark the object on the context for deletion. Note that the table row in Common Data Service is not deleted until the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext>.<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceContext.SaveChanges> method is called.  
