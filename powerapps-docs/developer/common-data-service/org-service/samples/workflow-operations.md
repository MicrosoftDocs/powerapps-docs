...
The `DeleteRequiredRecords()` method displays an option in the console window to delete any rows created by the sample(s).

The deletion is optional in case you want to examine the table instances (rows) created by the sample(s). Typically, you would not respond to the delete prompt in the console window until after you view the new organization rows in your browser. You can manually delete the created rows any time after the program terminates to achieve the same result.
