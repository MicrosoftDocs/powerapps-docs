...
1. Run the Plug-in Registration tool and register the assembly in the Common Data Service server's sandbox and database for the `PreOperation` stage of the `RetrieveMultiple` message for the `Account` table. 
...
When executed, the plug-in will ensure that inactive account rows will not be returned for the most common types of queries: <xref:Microsoft.Xrm.Sdk.Query.QueryExpression> and <xref:Microsoft.Xrm.Sdk.Query.FetchExpression>.
...
1. Use the recursive `RemoveAttributeConditions` method to look for any <xref:Microsoft.Xrm.Sdk.Query.ConditionExpression> instances that test the statecode column and remove them.
...
if (queryByAttributeQuery != null)
{
    ...
    //Query by column doesn't provide a complex query model that 
    // can be manipulated
}
```
...
By specifying a specific criteria, in this case the `address1_city` column value, which only one active row will match, these queries will return just that row.

Then, deactivate that row and run this code a second time. No rows will be returned.
...
try
{
    ...
    string no_records_message = "No rows returned";
