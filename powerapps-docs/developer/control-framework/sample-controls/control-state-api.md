...
The PowerApps Control Framework (PCF) allows you to persist state of a control across multiple renderings of the control within the same session.This provides you with the ability to build controls that can maintain user state throughout the user's session as the user navigates to and from the control.  
...
<manifest>
	<control namespace="SampleNamespace" constructor="TSControlStateAPI" version="1.0.0" display-name-key="TS_ControlStateAPI" description-key="TS_ControlStateAPI_Desc" control-type="standard">
		...
		<resources>
			<code path="TS_ControlStateAPI.js" order="1" />
...
export class TSControlStateAPI implements ControlFramework.StandardControl<InputsOutputs.IInputs, InputsOutputs.IOutputs>
{
...
private  _stateDictionary : ControlFramework.Dictionary = {};
