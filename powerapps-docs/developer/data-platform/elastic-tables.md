...
{
    CreateAttributeRequest createjsonAttributeRequest = new CreateAttributeRequest
    {
...
> [!NOTE]
> Although elastic table supports having One-to-Many relationship and fetch related rows using fetchXml or Odata WebAPI $expand, adding filters on related tables is not supported. This is because elastic tables cannot perform join operations between two tables.
...
### Create column with Json format
...
```csharp
public static OrganizationResponse CreateJsonAttribute(IOrganizationService service)
{
    CreateAttributeRequest createjsonAttributeRequest = new CreateAttributeRequest
    {
        EntityName = "contoso_sensordata",
...
{
    OrganizationRequest executeCosmosSqlQueryRequest = new OrganizationRequest("ExecuteCosmosSqlQuery");
    
    executeCosmosSqlQueryRequest.Parameters["EntityLogicalName"] = "cr03e_hyperdocument";
    executeCosmosSqlQueryRequest.Parameters["QueryText"] = "select c.sensorvalue.type, c.sensorvalue.value, c.deviceid * from c where c.props.sensorvalue.type='Humidity'";

    OrganizationResponse organizationResponse = service.Execute(executeCosmosSqlQueryRequest);

    // Deserialized query result into a class with expected schema.
    Entity result = (Entity)organizationResponse.Results["Result"];    
