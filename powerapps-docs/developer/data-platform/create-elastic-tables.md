...
- Formatted values for lookup columns aren't returned when the following conditions are true:
    ...
    - You're using a custom elastic table `partitionid` value. In other words, the `partitionid` value is set to something other than the default value (null). [Learn how to choose a partitionid value](elastic-tables.md#choose-a-partitionid-value).
