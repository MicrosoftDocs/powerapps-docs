...
description: Learn how to use FetchXml to filter rows when you retrieve data from Microsoft Dataverse. Explore operators, link-entity filters, and column comparisons.
ms.date: 03/09/2026
ms.reviewer: jdaly
ms.topic: how-to
author: MsSQLGirl
...
Each `condition` has an `operator` attribute to evaluate a row column value. You can choose from many [condition operator values](reference/operators.md).
...
You can also represent the previous query by using the [in operator](reference/operators.md#in) with a single `condition` element. This condition contains multiple [value elements](reference/value.md) to specify the values to compare to `address1_city`.
...
For example, use the [eq-userid operator](reference/operators.md#eq-userid) to evaluate any unique identifier and determine if it matches the calling user's ID.
...
For example, use the [eq operator](reference/operators.md#eq) to evaluate the `statecode` choice column value of a record by setting the `value` attribute.
...
For example, use the [between operator](reference/operators.md#between) to evaluate a number to determine if it is between a set of values.
...
You can create filters that compare columns on values in the same row by using the `valueof` attribute. For example, if you want to find any contact records where the `firstname` column value matches the `lastname` column value, use this query:
...
By using only FetchXML, you can compare field values in related tables. The following example returns rows where the contact `fullname` column matches the account `name` column.
...
These kinds of filters have the following limitations:
...
- You can compare only two columns at a time.
...
When you use these link types inside of a [filter element](reference/filter.md), these filters are child conditions that follow the behavior defined by the `type` attribute of the parent `filter`.

Filters that use these types return the parent row at most once, even if multiple matching rows exist in the link entity. They don't return column values from the link entity rows.
...
> [!NOTE]
> The meaning of `all` and `not all` link types is the opposite of what the names might imply, and they're typically used with inverted filters:
>
...
You can include up to 500 total [condition](reference/condition.md) and [link-entity](reference/link-entity.md) elements in a FetchXML query. Otherwise, you see this error message:
...
You need to reduce the number of conditions to execute the query. You might be able to reduce the number of conditions by using the [in operator](reference/operators.md#in) that you can use with numbers, unique identifiers, and strings up to 850 characters.
