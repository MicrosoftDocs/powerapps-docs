...
> [!NOTE]
> This article introduces common concepts related to connecting to Dataverse using OAuth with authentication libraries. This content focuses on how a developer can connect to Dataverse but not on the inner workings of OAuth or the libraries. For complete information related to authentication, see the Microsoft Entra ID documentation. The article [What is authentication?](/azure/active-directory/develop/authentication-scenarios) is a good place to start.
>
> Samples we provide are preconfigured with appropriate registration values so that you can run them without generating your own app registration. When you publish your own apps, you must use your own registration values.
...
If your app uses Server-to-Server (S2S) authentication, this step isn't required. That configuration requires a specific system user and the operations performed by that user account rather than any user that must be authenticated.

### Use Client Secrets & Certificates

For server-to-server scenarios, there isn't an interactive user account to authenticate. In these cases, you need to provide some means to confirm that the application is trusted. Confirmation is done using client secrets or certificates.

For apps that are registered with the **Web app /API** application type, you can configure secrets. These secrets are set using the **Keys** area under **API Access** in the **Settings** for the app registration.
...
> [!NOTE]
> Microsoft Authentication Library (ADAL) is no longer actively receiving updates and is supported only until June 2022. MSAL is the recommended authentication library to use for projects.
...
The following example is the minimum amount of code needed to execute a single Web API request, but it isn't the recommended approach. The example code uses the MSAL library and is taken from the [QuickStart](https://github.com/microsoft/PowerApps-Samples/tree/master/dataverse/webapi/CSharp/QuickStart) sample.
...
The recommended approach is to implement a class derived from  will<xref:System.Net.Http.DelegatingHandler> to be passed to the constructor of the <xref:System.Net.Http.HttpClient>. This handler allows you to override the <xref:System.Net.Http.HttpClient>.<xref:System.Net.Http.HttpClient.SendAsync*> method so that the access token is refreshed by the `AcquireToken*` method calls with each request sent by the Http client.
...
When using this `OAuthMessageHandler` class, the simple `Main` method would look like this.
...
The configuration string values moved into an App.config file connection string, and the Http client is configured in the `GetHttpClient` method.
...
Some apps you'll create aren't intended to run interactively by a user. For example, you may want to make a web client application that can perform operations on Dataverse data, or a console application that performs a scheduled task of some kind.

While you could achieve these scenarios using credentials for an ordinary user, that user account would need to use a paid license. This approach isn't recommended.

In these cases, you can create a special application user that is bound to a Microsoft Entra ID registered application. Next, use either a key secret configured for the app or upload a [X.509](https://www.itu.int/rec/T-REC-X.509/en) certificate. Another benefit of this approach is that it doesn't consume a paid license.

### Requirements to connect as an app

To connect as an app you'll need:
...
> [!IMPORTANT]
> After you save the configuration changes, a secret value is displayed. Be sure to copy the secret value for use in your client application code, as that value isn't accessible once you leave the page.
...
The first thing you must do is create a custom security role that defines what access and privileges this account has within the Dataverse organization. More information: [Create or configure a custom security role](/power-platform/admin/database-security#create-or-configure-a-custom-security-role)

After you created the custom security role, you must create the user account that will use it.
...
If you're connecting using a client secret and using the <xref:Microsoft.Xrm.Tooling.Connector>.<xref:Microsoft.Xrm.Tooling.Connector.CrmServiceClient> you can use similar code as shown here:
...
If you're connecting using a certificate and using the <xref:Microsoft.Xrm.Tooling.Connector>.<xref:Microsoft.Xrm.Tooling.Connector.CrmServiceClient> you can use similar code as shown here:
