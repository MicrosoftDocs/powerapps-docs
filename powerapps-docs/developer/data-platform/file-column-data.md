---
title: "Use File Column Data in Microsoft Dataverse"
...
contributors:
 ...
 - trin-msft
---
# Use file column data in Microsoft Dataverse

File columns in Microsoft Dataverse differ from other system columns that store binary data because you can't directly set the values in a create or update operation, or retrieve the file data with the record. To create, retrieve, update, or delete binary data for file columns, use the methods described in this article. Learn how to upload, download, and delete files using the SDK for .NET and the Web API, including support for chunked transfers of large files.

You can work with file column data by using several different methods through Web API. All methods are supported equally, so choose the method that works best for you. Because binary files might be large, you often need to split the file into multiple chunks (or blocks) that you can send or receive sequentially or in parallel to improve performance.

## File name column

Each file column has a supporting read-only string column that contains the name of the file. The schema name for this column has the same name as the file column, but with `_Name` appended to it. So if the schema name is `sample_FileColumn`, the supporting string column is `sample_FileColumn_Name`. The logical name for the supporting column is `sample_filecolumn_name`.

> [!NOTE]
> The file name column doesn't appear in the [Power Apps](https://make.powerapps.com/?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc) designer.
...
When you retrieve a record and include a file column, the value returned is a unique identifier for the file. You can use this value to delete the file by using the `DeleteFile` message. There's no other use for this ID other than to check whether the column has a value. For more information, see [Use the DeleteFile message](#use-the-deletefile-message).
...
### Retrieve extra information about files for a record

You can use the relationship between the file column table and the `FileAttachment` table to return information about all file columns associated with that table row.
...
The following request returns information about all the file columns that contain data related to the `account` record specified by using `accountid`.
...
You can upload files to a file column in three different ways:

- Use Dataverse messages available to both the SDK and Web API
- Upload a file in a single request by using Web API
- Upload the file in chunks by using Web API

> [!NOTE]
> - Verify whether the column maximum file size is large enough to accept the file you're uploading. For more information, see [Check maximum file size](#check-maximum-file-size).
> - You can also use these APIs to upload image column data. For more information, see [Use image column data](image-column-data.md).

### Use Dataverse messages to upload a file

You can use Dataverse messages by using the SDK for .NET or Web API. Uploading a file this way requires using a set of three messages:

|Message|Description|
|---------|---------|
|`InitializeFileBlocksUpload`|Use this message to indicate the column that you want to upload a file to. It returns a *file continuation token* that you can use to upload the file in blocks by using the `UploadBlock` message and with `CommitFileBlocksUpload`.|
|`UploadBlock`|Split your file into blocks and generate a *blockid* for each block. Then make multiple requests until all the blocks are sent together with the file continuation token.|
|`CommitFileBlocksUpload`|After you send requests for all the blocks by using `UploadBlock`, use this message to commit the upload operation by sending:<br />- The list of generated blockids<br />- The name of the file<br />- The [MIME type](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/MIME_types) of the file<br />- The file continuation token|

#### [SDK for .NET](#tab/sdk)

To upload a file or image, use a function like the following one that uses the [InitializeFileBlocksUploadRequest](xref:Microsoft.Crm.Sdk.Messages.InitializeFileBlocksUploadRequest), [UploadBlockRequest](xref:Microsoft.Crm.Sdk.Messages.UploadBlockRequest), and [CommitFileBlocksUploadRequest](xref:Microsoft.Crm.Sdk.Messages.CommitFileBlocksUploadRequest) classes.
...
The response is a [InitializeFileBlocksUploadResponse ComplexType](xref:Microsoft.Dynamics.CRM.InitializeFileBlocksUploadResponse) that provides the `FileContinuationToken` value to use with subsequent requests.
...
You must then break up the file into blocks of 4 MB or less and send each block by using the [UploadBlock Action](xref:Microsoft.Dynamics.CRM.UploadBlock) with the following properties:


|Property|Description:  |
|---------|---------|
|`BlockId`|A valid Base64 string value that identifies the block. Before encoding, the string must be less than or equal to 64 bytes in size.<br />For a given file, the length of the `BlockId` value must be the same size for each block.|
|`BlockData`|A Base64 encoded string containing the byte[] that's less than 4 MB in size representing the portion of the file being sent.|
...
After you send all the parts of the file by using multiple requests that use the [UploadBlock Action](xref:Microsoft.Dynamics.CRM.UploadBlock), use the [CommitFileBlocksUpload Action](xref:Microsoft.Dynamics.CRM.CommitFileBlocksUpload) to complete the upload by using these properties:

|Property|Description:  |
|---------|---------|
|`FileName`|The name of the file.|
|`MimeType`|The [MIME type](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/MIME_types) of the file.|
|`BlockList`|An array of strings representing the `BlockId` values in the previous `UploadBlock` operations. The order of these strings represents the order the server uses to combine the blocks into the original file.|
|`FileContinuationToken`|The value of the `InitializeFileBlocksUploadResponse.FileContinuationToken`|
...
The [CommitFileBlocksUploadResponse ComplexType](xref:Microsoft.Dynamics.CRM.CommitFileBlocksUploadResponse) returns the `FileSizeInBytes` and the `FileId` value. You can use the `FileId` value to delete the file by using the [DeleteFile Action](xref:Microsoft.Dynamics.CRM.DeleteFile).
...
If the size of the file is less than 128 MB, you can upload the file in a single request by using the Web API.
...
The following PowerShell `Set-FileColumn` function demonstrates how to upload a file in a single request by using Web API.

To learn more about using PowerShell and Visual Studio Code with the Dataverse Web API, see:
...
This function requires a connection that sets the global `$baseURI` and `$baseHeaders`. Set these values by using the `Connect` function described in [Create a Connect function](webapi/use-ps-and-vscode-web-api.md#create-a-connect-function).
...
To upload your file in chunks by using the Web API, use the following set of requests.
...
Set the file name by using the `x-ms-file-name` query parameter.
...
> [!NOTE]
> You can also include the file name as a `x-ms-file-name` request header, but this approach doesn't support file names outside the ASCII character set. If you use the header, it takes precedence over the `x-ms-file-name` query parameter.
...
The response [Location](https://developer.mozilla.org/docs/Web/HTTP/Headers/Location) header includes a URL to use in subsequent requests. It includes a `sessiontoken` query parameter that indicates that all the requests sent by using it are part of the same operation.
...
| [Content-Range](https://developer.mozilla.org/docs/Web/HTTP/Headers/Content-Range) | Using this format: <br />`<unit> <range-start>-<range-end>/<size>`<br />The value of the first request: `bytes 0-4194303/25870370` indicates that the measurement is using bytes. This request includes the first `4194303` bytes of a file that is `25870370` bytes (almost 25 MB) in size.<br />Each subsequent request increases this value until the entire file is sent:<br />`bytes 4194304-8388607/25870370`<br />`bytes 8388608-12582911/25870370`<br />`bytes 12582912-16777215/25870370`<br />`bytes 16777216-20971519/25870370`<br />`bytes 20971520-25165823/25870370`<br />`bytes 25165824-25870369/25870370`<br />|
| [Content-Length](https://developer.mozilla.org/docs/Web/HTTP/Headers/Content-Length) | Indicates the size of the message. In the preceding example, this value for the last request is `704546` rather than `4194304`. |
...
For each request that contains partial content, the response is [206 PartialContent](https://developer.mozilla.org/docs/Web/HTTP/Status/206).
...
For the final request that includes the last chunk of the file, the response is [204 NoContent](https://developer.mozilla.org/docs/Web/HTTP/Status/204).
...
The following PowerShell `Set-FileColumnInChunks` function demonstrates how to upload a file in chunks. This function requires a connection that sets the global `$baseURI` and `$baseHeaders` by using the `Connect` function described in [Create a Connect function](webapi/use-ps-and-vscode-web-api.md#create-a-connect-function).
...
Before you upload a file, check whether the size of the file exceeds the configured **Maximum file size** stored in the `MaxSizeInKB` property.
...
Use the following examples to check the maximum file size:
...
You can use three different methods to download files from a file column:

- Use Dataverse messages available to both the SDK and Web API
- Download a file in a single request by using Web API
- Download the file in chunks by using Web API

> [!NOTE]
> You can also use these methods to download image columns, but there are some differences. For more information, see [Download images](image-column-data.md#download-images).
> 
> For on-premises environments or when an environment uses a [Self-managed key (BYOK)](/power-platform/admin/manage-encryption-key), the file isn't in file storage. When a file isn't in file storage, downloading in multiple chunks isn't supported. The [InitializeFileBlocksDownloadResponse ComplexType](xref:Microsoft.Dynamics.CRM.InitializeFileBlocksDownloadResponse) and [InitializeFileBlocksDownloadResponse Class](xref:Microsoft.Crm.Sdk.Messages.InitializeFileBlocksDownloadResponse) have an `IsChunkingSupported` property that indicates if the file can be downloaded in multiple chunks or not. If chunking isn't supported, the `BlockLength` value is set to the file size.
> 
> Trying to download a partial chunk when `IsChunkingSupported` is set to false results in this error:<br />
> 
...
You can use Dataverse messages by using the SDK for .NET or Web API. Downloading a file this way requires using a pair of messages:

|Message|Description|
|---------|---------|
|`InitializeFileBlocksDownload`|Use this message to indicate the column that you want to download a file from. It returns the file size in bytes and a *file continuation token* that you can use to download the file in blocks by using the `DownloadBlock` message.|
|`DownloadBlock`|Request the size of the block, the offset value, and the file continuation token.|

After you download all the blocks, you must join them to create the entire downloaded file.

#### [SDK for .NET](#tab/sdk)

You can use a function like the following to download a file or image by using the SDK and the [InitializeFileBlocksDownloadRequest](xref:Microsoft.Crm.Sdk.Messages.InitializeFileBlocksDownloadRequest) and [DownloadBlockRequest](xref:Microsoft.Crm.Sdk.Messages.DownloadBlockRequest) classes.
...
Based on the size of the file and the size of the block you want to download, send more requests by using the [DownloadBlock Action](xref:Microsoft.Dynamics.CRM.DownloadBlock) as shown in the following section.

> [!NOTE]
> The example in the following section is for cases when `IsChunkingSupported` is set to true. If it is false, set `BlockLength` to full file size.
...
| `x-ms-file-name` | The name of the file. |
|`mimetype`|The [MIME type](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/MIME_types) of the file.|
|[Access-Control-Expose-Headers](https://developer.mozilla.org/docs/Web/HTTP/Headers/Access-Control-Expose-Headers)|Indicates that the `x-ms-file-size`, `x-ms-file-name`, and `mimetype` header values are available to scripts running in the browser, in response to a cross-origin request.|
...
> [!NOTE]
> The following example works when `IsChunkingSupported` is set to true. If it's false, use [Download a file in a single request using Web API](#download-a-file-in-a-single-request-using-web-api).

To download your file in chunks by using the Web API, use the following set of requests.
...
Use the [Range](https://developer.mozilla.org/docs/Web/HTTP/Headers/Range) header to specify the number of bytes to return by using this format:<br />
`<unit>=<range-start>-<range-end>`<br />
Where the `unit` is bytes and the `range-start` for the first request is `0`.

You don't know how many iterations are required to download the entire file until you get the first response where the `x-ms-file-size` response header tells you the size of the file.
...
| `x-ms-chunk-size` | Provides a recommended chunk size in bytes. |
|`mimetype`|The [MIME type](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/MIME_types) of the file.|
|[Access-Control-Expose-Headers](https://developer.mozilla.org/docs/Web/HTTP/Headers/Access-Control-Expose-Headers)|Indicates that the `x-ms-file-size`, `x-ms-file-name`, `x-ms-chunk-size`, and `mimetype` header values should be available to scripts running in the browser, in response to a cross-origin request.|
...
You can delete files in a file column by using either of the following methods:

- Use the Dataverse `DeleteFile` message, which is available to both the SDK and Web API.
- Send a DELETE request by using Web API to the file column of the record.

### Use the DeleteFile message

By using the unique identifier returned from `CommitFileBlocksUploadResponse.FileId` or retrieved from the column as described in [Behavior when retrieving](#behavior-when-retrieving), you can delete the file by using the `DeleteFile` message.

#### [SDK for .NET](#tab/sdk)

Use a function like the following example to delete a file by using the unique identifier and the [DeleteFileRequest Class](xref:Microsoft.Crm.Sdk.Messages.DeleteFileRequest).
...
Use the following request to delete a file by using the ID and the Web API [DeleteFile Action](xref:Microsoft.Dynamics.CRM.DeleteFile).
...
By using the Web API, you can delete a file by sending a `DELETE` request to the location of the file resource. 
