...
In the Power Platform, when User Local dates and times are submitted through the UI, an automatic calculation is done to set the data to the correct date and time. It will perform a calculation to change any submitted date to the corresponding UTC value based on column and UI settings.
However, submitting a date and time value to a User Local field using a direct oData API, this calculation does not occur and will result in unexplained data displays.
For example, if you are in the Pacific time zone and you submit 4/4/2021 12:00 here is what will happen:
...
Submitting through the UI: UI is set to User Local, Column is set to User Local
-   **Original Value:** 4/4/2021 12:00 in the Pacific time zone
-   **Value calculated to UTC and stored in Dataverse:** 4/4/2021 12:00 + 8:00 =  4/4/2021T20:00:00Z –this is because PST is -8:00 from UTC, so +8 is added to the stored value by the UI. 
-   **Value when displayed in UI by a user in the Pacific time zone:** 4/4/2021 12:00. The -8:00 UTC offset calculation is applied by the UI to 4/4/2021T20:00:00Z for the correct value.

Submitting through the API: UI is set to User Local, Column is set to User Local
-   **Original Value:** 4/4/2021T12:00:00 or 4/4/2021T12:00:00Z – no offset or UTC indicatorprovided, submitter is in the Pacific Time Zone
...
-   **Value when displayed in UI by a user in the Pacific time zone:** 4/4/2021 4:00. The -8:00 UTC offset calculation is applied by the UI on the value in Dataverse

To prevent this issue when using direct API calls to input data to User Local columns, you will need to calculate the offset of the user submitting the data and apply the offset. Using the above example: 
...
If you choose include the offset calculation, you cannot include the Z which is a UTC indicator and will prevent the offset from being accepted by the system.
