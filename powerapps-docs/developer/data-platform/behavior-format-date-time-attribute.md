...
In Web Client, time zone conversion was not done on the client side. If a user enters `2018-10-15 07:30` in a Web Client form, the Client API `Xrm.Page.getAttribute(<column name>).getValue()` will also return `2018-10-15 07:30`. This value is sent to the server and time zone adjustments happen there when saving the value.

In Unified Client, time zone conversion is done on the client side. If a user in UTC+8 time zone enters `2018-10-15 07:30` in a Unified Client form, the Client API `formContext.getAttribute(<column name>).getValue()` will return the adjusted value `2018-10-14T23:30:00Z`. The server will accept the value as is and not perform further time zone adjustments.
...
- Change the column behavior from `UserLocal` to `TimeZoneIndependent` so that entered times are not adjusted. Of course, this is only possible if time zones are not applicable for the column.
