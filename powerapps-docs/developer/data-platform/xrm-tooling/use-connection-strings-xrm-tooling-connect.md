...
|`ClientId`, `AppId` or `ApplicationId`|Specifies the `ClientID` assigned when you registered your application in Entra ID or Active Directory Federation Services (AD FS).|
|`ClientSecret` or `Secret` |Required when Auth Type is set to `ClientSecret`. Client Secret string to use for authentication.|
|`RedirectUri` or `ReplyUrl`|Specifies the redirect URI of the application you registered in Entra ID or Active Directory Federation Services (AD FS).<br /><br /> This parameter is applicable only when the authentication type is specified as `OAuth`.|  
...
> [!NOTE]
> OAuth is the preferred auth type for connecting to Dataverse when using an interactive flow.  This auth type fully supports the features of Entra ID Conditional Access and Multi-Factor authentication.
...
> [!NOTE]
> OAuth is the preferred auth type for connecting to Dataverse when using a interactive flow. This auth type fully supports the features of Entra ID Conditional Access and Multi-Factor authentication.
