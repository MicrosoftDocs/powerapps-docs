...
For both create or update operations you will get the same response:
...
Notice how the `OData-EntityId` response header uses the key values rather than the GUID primary key identifier for the record. Because of this, it is not recommended to include the key values within the body of the request for the following reasons:

- When an upsert represents an update operation, these values will be ignored.
- When an upsert represents a create operation, the key values in the URL will be set for the record if they are not present in the body. However, if the values in the body are different than the ones in the URL, they will override the URL values and the `OData-EntityId` response header will return a 404 if used to retrieve the created record.
