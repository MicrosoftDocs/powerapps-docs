...
|---------|---------|---------|
|`CallerObjectId`|User Entra ID Object ID|Use this header to impersonate another user when the caller has the privileges to do so. Set the value to the Entra ID Object ID of the user to impersonate. This data is in the [User (SystemUser) table/entity](../reference/entities/systemuser.md) [AzureActiveDirectoryObjectId](../reference/entities/systemuser.md#BKMK_AzureActiveDirectoryObjectId) attribute (column). More information: [Impersonate another user using the Web API](impersonate-another-user-web-api.md)|
