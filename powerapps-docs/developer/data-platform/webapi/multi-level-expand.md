...
You can retrieve results with deeper hierarchy by including multi-level `$expand` system query option with your query. When you use multi-level `$expand` with a collection-valued navigation property, the shape of the results is different than when you use only a single `$expand`.
...
- Each of the three account records have the single-valued navigation property `ownerid` set to the user FirstName LastName. The partner collection-valued navigation property is `user_accounts`.
- The three task records have the single-valued navigation property `regardingobjectid` set to Litware. The partner collection-valued navigation property is `Account_Tasks`.
...
```http
GET [Organization Uri]/api/data/v9.2/accounts?$filter=_ownerid_value eq 4026be43-6b69-e111-8f65-78e7d1620f5e&$select=name&$expand=Account_Tasks($select=subject) HTTP/1.1
...
```http
GET [Organization Uri]/api/data/v9.2/systemusers(4026be43-6b69-e111-8f65-78e7d1620f5e)/user_accounts?$select=name&$expand=Account_Tasks($select=subject) HTTP/1.1
Prefer: odata.maxpagesize=2
```
...
This query returns a collection of account records but the filtering is done using the `user_accounts` collection-valued navigation property. Again, because it only includes a single `$expand`, there is no paging provided for the expanded collections.

In this case, there are no `Account_Tasks@odata.nextLink` urls. This is because [TODO: Need explaination here].
...
```http
GET [Organization Uri]/api/data/v9.2/systemusers?$filter=systemuserid eq 4026be43-6b69-e111-8f65-78e7d1620f5e&$select=fullname&$expand=user_accounts($select=name;$expand=Account_Tasks($select=subject)) HTTP/1.1
Prefer: odata.maxpagesize=2
If-None-Match: null
```
...
In this case, there is more than 1 `$expand` system query option in the query, so the behavior is different.
...
- `user_accounts@odata.nextLink` returns the 2 accounts: Adventure Works (sample) and Fabrikam, Inc. (sample)
- `@odata.nextLink` returns no records because the query used the primary key for the `systemuser` entityset: `$filter=systemuserid eq 4026be43-6b69-e111-8f65-78e7d1620f5e`. In this case, Dataverse is anticipating that there could be more records and provides a paging URL. When you perform an GET request using that `URL`, it will contain no records and there will be no `@odata.nextLink`. Then you will know that there are no more records that meet the criteria.
...
```http
GET [Organization Uri]/api/data/v9.2/systemusers(4026be43-6b69-e111-8f65-78e7d1620f5e)?$select=fullname&$expand=user_accounts($select=name;$expand=Account_Tasks($select=subject)) HTTP/1.1
Prefer: odata.maxpagesize=2
If-None-Match: null
```
