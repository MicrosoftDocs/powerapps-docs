...
- There's a 2-minute time limit for plug-ins to execute. This limit is generous considering the impact that a plug-in can have on the performance of an application. Most synchronous plug-ins should complete in less than 2 seconds.
...
- Currently the only recommended approach for use in plug-ins.
...
- The primary performance benefit of using this approach is that less total data is transferred over the wire in the body of the request compared to sending the requests individually.
...
- If you're performing `Create` or `Update` operations on large numbers of records for a single table, we recommend using the `CreateMultiple` and `UpdateMultiple` messages when they become available for the tables you're using. You'll need to experiment to find the optimum number of items to send in each request.
