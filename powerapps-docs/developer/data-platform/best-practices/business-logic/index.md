...
contributors:
 ...
 - phecke
---
# Best practices and guidance for plug-in and workflow development for Microsoft Dataverse


This list contains guidance and best practices for plug-in and workflow development within Dataverse.
...
|[Don't duplicate plug-in step registration](do-not-duplicate-plugin-step-registration.md)     |Duplicate plug-in step registration cause the plug-in to fire multiple times on the same message or event.         |
...
|[Implement all types of queries when filtering results using `PreOperation` `RetrieveMultiple`](implement-all-types-of-queries-when-filtering-preoperation-retrievemultiple.md)|For best performance and consistent results for all applications, implement filtering for all types of queries that can be used with plug-ins that are registered for the PreOperation stage of `RetrieveMultiple`.|
|[Include filtering attributes with plug-in registration](include-filtering-attributes-plugin-registration.md)     |If you don't set filtering attributes for a plug-in registration step, the plug-in executes every time an update message occurs for that event.         |
...
|[Manage plug-ins in single solution](manage-plug-ins-single-solution.md)|Maintain the definition of a plug-in assembly within a single solution. |
|[Optimize custom assembly development](optimize-assembly-development.md)     |Consider merging separate plug-ins and custom workflow activities into a single custom assembly to improve performance and maintainability. Move plug-ins and custom workflow activities into multiple custom assemblies if an assembly size is near the sandbox assembly size constraints.         |
|[Set KeepAlive to false when interacting with external hosts in a plug-in](set-keepalive-false-interacting-external-hosts-plugin.md)     |Setting the `KeepAlive` property to `true` in the HTTP request header or not explicitly defining it as `false` can cause increased execution times of plug-ins.         |
...
|[Verify certification dependencies for plug-ins making outbound calls](verify-certification-dependencies.md)|Ensure that any certificates your code depends on for outbound calls have a valid chain of certificates.|
|[Write memory efficient code](/troubleshoot/power-platform/dataverse/plug-in-execution/dataverse-plug-ins-errors#worker-process-reaches-the-memory-limit)|Retrieve only data within your plug-in that is needed.|
