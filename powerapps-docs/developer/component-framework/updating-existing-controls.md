...
Once the project gets created, migrate your code component source to the new project:
...
1. The `code` resource entry must now point to the pre-compiled source file of the code component. The file name defaults to **index.ts**.
...
1. Edit the `code` entry in `ControlManifest.Input.xml` to the pre-compiled source file of your code component (typically index.ts).
2. Edit any paths of the resources to refer to the relative paths to files on the disk correctly.
...
The build tools expect the component source to be exported using standard ES6 module format. Legacy components typically get exported as internal modules (aka namespaces). The component source file must be modified as below, to align with the new build tools.
