---
title: Add code components to a column or table | Microsoft Docs
...
Code components let you transform columns that traditionally contain text into visualizations. Similarly, you can use code components to transform datasets, such as a view, to display in a more visual rendering rather than a list of rows. Code components can appear as visualizations on forms, dashboards, views, and homepage grids. 


   > [!div class="mx-imgBorder"] 
   > ![Custom slider control](../../maker/model-driven-apps/media/slider-control.PNG "Slider control for a column")
...
2. Expand **Tables**, expand the table that you want, such as the **Opportunity** table, select **Forms**, and then open a form such as the **Main** form.
...
To add a code component like data-set component or simple table component to a grid or view, follow the steps below:
  ...
  - Click on the arrow next to **Tables** tab a select the table you want to add the code component. 
...
## See the code component in action  
 ...
 Open a row that includes the column with the code component, such as the Opportunity form from the previous example, and view how the column is changed. The column is now rendered as a slider component instead of the text column.  
