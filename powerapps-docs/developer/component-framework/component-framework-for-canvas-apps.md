...
To delete a code component from a canvas app, select the code component that you want to delete and press the **Delete** button on the menu. When the code component is deleted from the app, all the code component elements get deleted from the app and the app package. 

## Updating existing code components

When you update the code components, we will specify the *version* attribute in the manifest file so that latest changes are reflected in the runtime. For canvas apps when you update the existing code components, there is no need to update the *version* attribute, as by design the canvas apps picks up the latest code component and displays it in runtime. Only single version of the same component can exist in canvas apps.
