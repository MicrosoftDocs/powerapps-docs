...
Add your SharePoint data source by following the instructions in [Connect to data](/power-apps/developer/code-apps/how-to/connect-to-data).
...
> [!NOTE]
> Generated models might include internal property names with # (ex: "Choices1#Id") that are used for binding in forms but shouldn't be included in the payload sent to the SharePoint connector. When updating or creating a row in your list, the SharePoint API expects you to provide the expanded object for referenced columns (author, editor, person/group columns, and so on) rather than just the IDs. 
