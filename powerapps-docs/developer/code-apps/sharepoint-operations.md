...
This guide helps developers use the Power Apps SDK to connect their code app to SharePoint and use the generated models and services to perform CRUD (Create, Read, Update, Delete) operations on a SharePoint list.  

## Prerequisites 
- Power Apps code apps SDK [@microsoft/power-apps - npm package](https://www.npmjs.com/package/@microsoft/power-apps) 
- Power Apps CLI (PAC CLI) version 1.50 or later 
...
- Add SharePoint lists as data sources using the PAC CLI 
...
Add your SharePoint data source using the instructions in [Connect to data](/power-apps/developer/code-apps/how-to/connect-to-data) 
...
You can import and use them in your App.tsx code like this: 
...
Use `update(id, payload)` from the generated service file. Provide the same expanded objects as you would when creating a record. Example: 
...
Ensure `recordId` is the string ID the service expects (often numeric item ID converted to string). 
...
To populate dropdowns, you can call `getReferencedEntity()`. SharePoint always returns a value array that contains objects from the referenced entity. You may opt to normalize the response, as some connectors return a structure in the format `{ value: [] }`, while others provide the array directly.  
...
The Power Apps SDK and the PAC CLI allow CRUD operations on SharePoint lists, but do not support Document Processing APIs or actions like item synchronization or permission changes. You can add these features by creating a custom service file for your code app. 
