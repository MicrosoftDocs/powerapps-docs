...
|---------|---------|
|power.config.json|A file generated from the Power Apps client library for code apps, which contains metadata. Both the CLI and Power Apps client library for code apps use this metadata for Power Platform connections and to publish an app to an environment. Your app logic isn't expected to interact with the `power.config.json` file.|
...
- The Power Apps client library for code apps exposes APIs that your code can use and the generated models and services your app uses to perform data requests via Power Platform connectors.
