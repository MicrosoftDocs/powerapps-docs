...
Charts display data for only those saved queries (organization-owned views) that use FetchXML (`SavedQuery.FetchXml` column) to filter the rows. If a saved query uses the query API (`SavedQuery.QueryAPI` column) to filter the rows, the chart will not appear for that saved query. This limitation is not applicable for user queries (user-owned views) because the user query table does not use the `QueryAPI` column to filter the rows.  
...
You can create and attach visualizations to only those tables in Common Data Service that support the new ribbon interface. This is because all of the chart controls are only present in the ribbon interface of Common Data Service. Custom tables are also supported for visualizations. You can turn off the visualization support for custom tables if you want to. However, you cannot disable visualization support for the default tables.  
