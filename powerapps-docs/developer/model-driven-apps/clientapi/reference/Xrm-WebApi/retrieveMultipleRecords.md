...
<td>Number</td>
<td>No</td>
<td><p>Specify a positive number that indicates the number of table rows to be returned per page. If you do not specify this parameter, the value is defaulted to the maximum limit of 5000 rows.</p> 
<p>If the number of rows being retrieved is more than the specified <code>maxPageSize</code> value or 5000 rows, <code>nextLink</code> column in the returned promise object will contain a link to retrieve the next set of tables. </td>
</tr>
<tr>
...
On success, returns a promise that contains an array of JSON objects (**entities**) containing the retrieved table rows and the **nextLink** column (optional) with the URL pointing to next set of rows in case paging (`maxPageSize`) is specified in the request, and the row count returned exceeds the paging value.
...
Xrm.WebApi.retrieveMultipleRecords("account", "?$select=name,primarycontactid&$filter=primarycontactid eq a0dbf27c-8efb-e511-80d2-00155db07c77").then(
    function success(result) {
        ...
        // perform additional operations on retrieved rows
    },
    function (error) {
        console.log(error.message);
        // handle error conditions
...
The following example demonstrates the use of the `maxPageSize` parameter to specify the number of rows (3) to be displayed in a page.
...
This example will display 3 rows and a link to the next page. Here is an example output from the **Console** in the browser developer tools:
...
Use the **$expand** system query option in the navigation properties to control the data that is returned from related entities. The following example demonstrates how to retrieve the contact for all the account rows. For the related contact rows, we are only retrieving the `contactid` and `fullname`:
...
For more examples of retrieving multiple rows using Web API, see [Query Data using the Web API](../../../../common-data-service/webapi/query-data-web-api.md).
