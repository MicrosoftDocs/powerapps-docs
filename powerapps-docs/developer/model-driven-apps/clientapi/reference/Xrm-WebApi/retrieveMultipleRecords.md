...
```
#### For mobile offline scenario
Similarly to the online scenario, use the **$expand** system query option to retrieve data from related tables. For navigation properties based on one-to-many and many-to-one relationships, the result will have the same schema as in the online scenario. Many-to-many relationships are not supported.
