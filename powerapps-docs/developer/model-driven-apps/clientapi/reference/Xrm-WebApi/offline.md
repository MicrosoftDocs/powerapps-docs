...
> [!NOTE]
> Use **Xrm.WebApi.offline** instead of the [deprecated](/dynamics365/get-started/whats-new/customer-engagement/important-changes-coming#some-client-apis-are-deprecated) **Xrm.Mobile.Offline** namespace to create and manage rows in the mobile clients while working in the offline mode.
...
> [!IMPORTANT]
> While creating or updating row in the offline mode, only basic validation is performed on the input data. Basic validation includes things such as ensuring that the table column name specified is in lower case and does exist for a table, checking for data type mismatch for the specified column value, preventing rows getting created with the same GUID value, checking whether the related table is offline enabled when retrieving related table rows, and validating if the row that you want to retrieve, update, or delete actually exists in the offline data store. Business-level validations happen only when you are connected to the server and the data is synchronized. A row is created or updated only if the input data is completely valid.
