...
GridCell of a selected grid row is analogous to a control on a form that is tied to an column in an editable grid. See [Collections (Client API reference)](../collections.md) for information on the methods available to access data in a collection.
...
|[setDisabled](../controls/setDisabled.md)| Sets whether the cell is disabled.<br/><br/>**NOTE**: Enabling a read-only cell for editing can cause an error when the row is saved. If the column is considered read-only by the server, an error may occur if the value is modified. This may happen in scenarios where the user doesn't have write privileges to the row, the row is disabled, or the user doesn't have the necessary field-level security privileges.| 
