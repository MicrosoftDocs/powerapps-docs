...
- `Font` – The font for your custom theme. The font that the custom theme renders depends on the browser and target machine's ability to show that font. 
...
Makers can customize the styling of the app header to deviate from the app theme. To make these customizations, extend the XML created for the custom modern theme by adding one or more of the following attributes inside an `AppHeaderColors` tag.
...
After publishing your custom theme, validate the application of the theme in the app to ensure everything appears as you expect. 
...
3. Apply custom app header colors to apps in your environment by assigning this web resource to the environment or app in the **Override app header color** setting. Follow the steps from [Applying custom themes to your environment](#apply-the-custom-theme-to-apps-in-your-environment) but use the **Override app header color** setting instead.
    > [!NOTE]
    > The **Override app header color** setting is ignored if the **Custom theme definition** setting is defined.  
1. Verify your custom app header visuals, including all button states, to ensure everything appears as you expect and there are sufficient contrast ratios for accessibility. Verify the following color choices:
...
When you play the app, the app header looks like the following image. You might need to refresh the browser tab to display the theme.  
