...
- `LockPrimary` – A boolean that determines how the selected seed color is used to generate the 16-slot color palette.  
    -  True (Default): The palette is created in a way that makes it accessible but doesn't guarantee the seed color will appear in any part of the generated palette. This is the default setting. Use the [Fluent theme generator]() to preview the generated palette based on your selections for basePaletteColor, vibrancy, and hueTorsion.  
    - False: The seed color occupies the central (primary) slot of the palette. The other colors are created by progressively lightening shades on one side and darkening them on the opposite side. The resulting palette may not satisfy contrast ratio accessibility standards.
...
<general documentation/guidance of each color slot> 
