...
|:--------------|:-------------------------|
|**DataSource** | Should be set to the data source connected to the parent table of the host model-driven form. <br />Automatically set when [embedding a new canvas app](embedded-canvas-app-add-classic-designer.md). When embedding an existing canvas app, ModelDrivenFormIntegration data source can be configured from the canvas app tree view. |
