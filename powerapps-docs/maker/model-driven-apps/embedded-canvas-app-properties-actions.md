...
|:--------------|:-------------------------|
|**DataSource** | Should be set to the data source connected to the parent table of the host model-driven form. <br />Automatically set when [embedding a new canvas app](embedded-canvas-app-add-classic-designer.md). When embedding an existing canvas app, ModelDrivenFormIntegration data source can be configured from the canvas app tree view. <br/> The value for DataSource should always refer to the table of the form the app is embedded in.  It should not be filtered or modified in any way.|
