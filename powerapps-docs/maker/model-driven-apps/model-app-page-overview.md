---
title: "Converge model-driven and canvas apps using the custom page in Power Apps"
...
A custom page is more flexible than a model-driven app form, view, or dashboard page. You can include one or more tables. Then, you define the data and component interactions. The custom page is a separate solution element, which means one maker can edit one custom page at a time. Like other model-driven app pages, the page state comes from the parameters passed or is retrieved from persisted tables.

> [!IMPORTANT]
> Custom pages are a feature with significant product changes and currently have a few known limitations outlined in [Custom Page Known Issues](model-app-page-issues.md).
...
| Authoring custom pages | General Availability | Use the modern app designer and Power Apps Studio to author custom pages that are supported at runtime. |
...
These images show the custom page inline within the model-driven app. The model-driven app has the full-page space in the images without the header and navigation.
...
By using a custom page, you can create a new page experience by using the canvas app capabilities. This approach provides a low-code authoring experience with more flexible layouts, more control with styling options, the ability to add connector data, use expressions, and more. You create custom pages in Power Apps Studio with increasing context of the model-driven app that the page runs in.
...
In most cases, use custom pages instead of embedded canvas apps for tighter integration and better performance. 

> [!NOTE]
> Don't exceed 25 custom pages in a model-driven app. If you add more custom pages, it can increase the wait time that an app user experiences when they play an app with custom pages, after the last app publish. The increased user wait time affects only the first app launch, by the first user, after the last publish of the model-driven app.

## Migrate standalone canvas app content to custom pages
...
To migrate an existing standalone canvas app, start by identifying a mapping of screens to separate custom pages. For each separate custom page, complete these steps:
...
* Do custom pages need to be shared like standalone canvas apps?
  ...
  The custom page aligns with the model-driven app page sharing, which relies on the model-driven app sharing without sharing individual pages for app users. Makers might need to share the custom page to allow editing.
