...
- All existing data flow functions supported 
- Imperative functions that work with data are supported
- Imperative functions for simple Confirm and Notify supported
...
## AutoSave
- Many JavaScript commands start by saving the form buffer
  - Which makes sense, it makes the rest of the code easier to work with
- By default, we do this on behalf of the maker
...
- **Selected** is provided by the host of the command.
...
## Navigate
> [!NOTE]
> Soon the navigate function will be available to interact with custom pages. See the client API reference to do that today using JavaScript.
...
The Message paramiter is displayed in the body of the dialog box. If the message is very long, it will either be truncated or a scroll bar provided.
...
Use the **Notify** function to display a banner at the top of the app that does not need to be dismissed.
