---
title: "Command designer overview in Power Apps"
description: "Overview of modern commanding, command designer, and Power Fx in Power Apps"
...
At a high level, command customization fits into three categories. Various capabilities exist within each category and are covered in more depth throughout the modern commanding documentation:  
...
- **Action**. The logic that runs when a user selects a button. For example, creating and updating data or interacting with various controls and pages inside the app.
...
- **Quick actions**. Quick actions are associated with the main grid location. To add or edit commands for both quick actions and main grid locations, select the desired table from within the modern app designer, then edit the command bar and choose the main grid location. The first five commands, determined by order, are also shown as quick actions when playing the app.
...
- **Command**. Standard button. Performs an action when selected. Can also be nested in groups within dropdowns and split buttons. These were called flyouts in classic commanding.
...
|Customized using |Hand editing XML within solution files or using non-Microsoft tools. Requires time-consuming solution export and import operations.|Command designer as well as Dataverse API support.|
...
|Use JavaScript|Yes|Yes. Now simpler. The same JavaScript can be used for classic and modern commands.|
...
|Table specific commands that display in all apps containing the table|Yes|Yes. Requires modifying the `appaction` definition within the solution file.|
...
|Dynamically populate a flyout with code|Yes|No. Create commands declaratively.|
...
Classic visibility rules are also supported within modern commands. However, support for classic rules was needed for reliably migrating classic commands to modern commanding and  classic rules customization within command designer isn't supported. We recommend you use Power Fx going forward.

|Use case|Classic Rule|Classic options|Power Fx visible property|
|:----|:----|:----|:----|
|Show or hide based on data values|CustomRule|Use JavaScript |!IsBlank(Self.Selected.Item.Email)|
|Show or hide based on table permission|EntityPrivilegeRule|Multiple|DataSourceInfo()|
|Show or hide based on record permission|RecordPrivilegeRule|Multiple|RecordInfo()|
...
|Show or hide based on form state. For example, show for the create form|FormStateRule|Create. Existing. ReadOnly. Disabled. BulkEdit|Self.Selected.State = FormMode.New|
|Show when > 1 records are selected in a grid|SelectionCountRule| |CountRows(Self.Selected.Items) > 1|
|Show or hide for a related table in a polymorphic lookup. For example, check whether the lookup is a user or a team|CustomRule|PrimaryEntityTypeCode|IsType(), AsType|
