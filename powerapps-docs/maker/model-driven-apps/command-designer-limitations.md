...
- Metadata changes for attributes might not reflect in the Power Fx experession. To update the metadata, open the component library using command bar actions in Command designer. In the canvas designer select Data sources, select the data source and refresh. This will update the metadata for the component library.
