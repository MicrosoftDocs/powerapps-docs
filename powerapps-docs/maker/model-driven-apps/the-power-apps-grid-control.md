...
1.	There are several properties included with the control to let you tailor the grid experience for that table. To modify any of these properties, select **Edit** in the corresponding row, and then change the value using the dropdown list under **Bind to static options**. 
    ...
    -  The **Use first column for lookup edits** property allows makers to select the field used for editing lookup cells. When set to **Yes** the cell will show and filter by the first column of the target entity's Lookup View; when set to **No** the cell will show and filter by the primary field. The default value is **No**.
