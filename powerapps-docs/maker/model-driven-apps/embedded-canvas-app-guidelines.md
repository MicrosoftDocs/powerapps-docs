...
- Adding canvas app on the sub-grid control is no longer available.
    - In the preview release, makers were able to add a canvas app on a sub-grid control. With canvas app embedding on model-driven forms now out of preview and generally available adding an embedded canvas app on a model-driven form is streamlined to the field. 
    ...
    - To access the list of related records in the canvas app, makers can use the Common Data Service connector and [*Filter*](../canvas-apps/functions/function-filter-lookup.md) function with the *Improve data sources experience and Common Data Service views* capability enabled in the canvas app.  
