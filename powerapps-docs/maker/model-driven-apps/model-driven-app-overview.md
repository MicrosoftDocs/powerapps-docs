...
Defining and enforcing consistent business processes is a key aspect of model-driven app design. Consistent processes help make sure your app users focus on their work and not on remembering to perform a set of manual steps. Processes can be simple or complex and often change over time. To create a process, in [solution explorer](#advanced-model-driven-app-making) on the left navigation pane select **Processes**, and then select **New**.  
