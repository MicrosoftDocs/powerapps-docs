...
1. Optionally, you can set the conditions to prompt entries in case they're specific to the page context, and so on. For example, this prompt entry checks if the page context's table type name matches specified values. If either condition is true, the Copilot chat is activated.
