...
1. **Start a conversation with your AI tool.** Describe what you want to create, including what data you want to include (which Dataverse tables or whether to create sample mock data). Be as specific as you want—the more vague you are with the request, the more details the agent will try to fill in itself. You can also attach or provide an image or other materials to help guide the visuals, theming, and layout. For example:
   ...
   - "Create a generative page for a sales pipeline visualization with opportunities using modern blue theme"
