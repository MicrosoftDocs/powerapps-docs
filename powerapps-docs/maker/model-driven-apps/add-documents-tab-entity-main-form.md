---
title: "Add the Documents tab to the main form for a table"
...
:::image type="content" source="media/document-files-tab.png" alt-text="Document files tab.":::
...
1. In your Power Platform development environment, create a new solution or open an existing solution. For more information about creating solutions, see: [Create a solution](../data-platform/create-solution.md).
1. Add the table to the solution or select an existing table. All standard and custom tables are supported. For more information about tables in solutions, see: [Add solution components](../data-platform/create-solution.md#add-or-remove-solution-objects)
1. Include the form for the table in the solution, such as the main form for the account table. Next to the table, select **...**, and then select **Edit**. Select the **Forms** area. If the required form is missing, add it.
1. Add a one-column tab to the main form. To do this, in the form designer select an area on the form canvas, select **Add Component**, and then select **1 Column Tab**.  
   :::image type="content" source="media/insert-one-column-tab.png" alt-text="Insert one column tab.":::
1. In the form designer select the **New Tab** on the form designer canvas, select **Add Column**, and add a column such as *Address 1: City* from the left pane. Any text or numeric column can be used.
   :::image type="content" source="media/add-field-to-tab.png" alt-text="Add a column to the tab.":::
...
1. Select **Save and Publish**, and then close the form designer.
...
1. Scroll down to the control id="*field name*" element, such as **control id="address1_city"** and replace the entire element with the [XML sample](#xml-sample-for-adding-the-documents-tab-to-a-form) in this topic.
    ...
    :::image type="content" source="media/form-xml.png" alt-text="XML sample insertion point.":::
...
1. Make these modifications to the XML sample.
     ...
     a. Locate the **RelationshipName** element and replace it with the schema name that appears as *entityLogicalName*_SharePointDocument. For example, for the accounts table the schema name for the relationship is Account_SharePointDocument, which is the schema name for the XML sample in this topic. To find the name for a different table, go to **Settings** > **Customizations** > **Customize the System** > **Tables** > select the table > select **1:N Relationships**. Locate the **Related Table** of type **SharePointDocument**.
      ...
      ::image type="content" source="media/account-sharepointdocument.png" alt-text="Account relationship SharePoint document.":::
     ...
     b. Create a globally unique identifier (GUID) and replace the existing **uniqueid** GUID located in the **control** element pasted in the previous step while preserving the curly braces {}.  
       ::image type="content" source="media/control-unique-id.png" alt-text="Control element unique id.":::
     c. Save the changes made to customizations.xml.

1. Open the solution.xml file and increment the **Version** element value. For example, change *1.1.0.0* to *1.2.0.0*.
1. Package all solution files into a compressed (zipped) folder and import it into the right environment. If an error appears then the previous solution should be removed. For more information on managing solutions see: [Import, update, and upgrade a solution](../data-platform/import-update-export-solutions.md)
...
1. Select the **Documents** tab, and then on the form designer toolbar, select **Delete**.
   ...
   :::image type="content" source="media/delete-files-tab.png" alt-text="Delete files tab":::
