---
title: "Migrate embedded canvas apps on model-driven forms"
description: Learn how to migrate your embedded canvas apps created by using the public preview release.
ms.custom: ""
ms.date: 02/20/2026
ms.reviewer: ""
ms.suite: ""
...
search.audienceType: 
  - maker
---
# Migrate embedded canvas apps on model-driven forms
...
2. Open the embedded canvas app created using the public preview release for editing in Power Apps Studio. For steps on editing a canvas app, go to: [Edit a canvas app](../canvas-apps/edit-app.md).
...
4. Copy the controls from the embedded canvas app created using the public preview release to the new embedded canvas app, one screen at a time using these steps:
...
5. When you finish copying all the screens, select the browser tab from Step 3, that has the new embedded canvas app created using the latest release.
...
7. Add any missing data sources in the new embedded canvas app.
8. Update all broken references in the new embedded canvas app. 
9. When you finish making changes, select the **File** tab, and then select **Save**.
10. To make your changes available to users, select **Publish** and then select **Publish this version**.
...
To migrate an embedded canvas app on a model-driven form that uses a list of rows related to the current (main form) row, follow these steps.
...
4. With recent updates Microsoft Dataverse now also provides support to use table views as a filter. Go to this blog post for details: [Improved data source selection and Dataverse views](https://powerapps.microsoft.com/blog/improved-data-source-selection-and-common-data-service-views/). 
