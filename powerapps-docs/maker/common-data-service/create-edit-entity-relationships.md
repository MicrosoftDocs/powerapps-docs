...
- Prevent deleting the primary table so that the related table records can be reconciled, perhaps by associating them with a different primary table.
...
Using Reparent and Share cascading behaviors are helpful when you want to provide access to records across related tables. But there can be a change in process or design that requires a change of the cascading behavior settings.
