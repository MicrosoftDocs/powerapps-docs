...
# Projects overview  
 ...
 Projects are built on the solution system in the Dynamics platform, learn more about the full capabilities of [solutions](). Projects can be leveraged to transport an app from one organization to another. You can get projects from [AppSource](https://appsource.microsoft.com/) or from an independent software vendor (ISV). A project can contain one or many apps as well as other components such as entities, option sets, etc. It can be exported and imported into an environment as an app or to apply a set of customizations to an existing app.  
...
-   When you customize the PowerApps environment directly, you work with a special project called the **Common Data Services Default Solution**. This project contains all the components in your system.  
  
-   You can export your **Default solution** project to create a backup of the customizations you have defined in your organization. This is good to have in a worst case scenario.  
  
<a name="BKMK_SolutionComponents"></a>   
### Project components  
 A project component represents something that you can potentially customize. Anything that can be included in a project is a project component. The following is a list of components that you can view in a project:  
...
### Managed and unmanaged projects  
 A **managed** project can be uninstalled after it is imported. All the components of that project are removed by uninstalling the project.  
...
> [!IMPORTANT]
>  Install an unmanaged project only if you want to add all the components to your default solution and overwrite any existing customizations.  
  
 Even if you don’t plan on distributing your apps or customizations, you might want to create and use an unmanaged project to have a separate view that only includes those parts of the application that you have customized. Whenever you customize something, just add it to the unmanaged projct that you created.  
...
### How solutions are applied  
 As mentioned above, projects are built on top of solution system in Dynamics platform. All solutions are evaluated as layers to determine what your app will actually do. The following diagram shows how managed and unmanaged solutions are evaluated and how changes in them will appear in your organization.  
