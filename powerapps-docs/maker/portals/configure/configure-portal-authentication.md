...
In Power Apps portals, each authenticated portal user is associated with a [contact record in Common Data Service](../../../developer/common-data-service/customer-entities-account-contact.md#contact-entity). Portal users must be assigned to [web roles](create-web-roles.md) to gain permissions beyond those granted to unauthenticated users. To configure permissions for a web role, configure its [webpage access and website access control rules](webpage-access-control.md). The portals feature allows portal users to sign in by using their choice of an external account based on [ASP.NET Identity](https://www.asp.net/identity). And in addition&mdash;though this authentication method isn't recommended<!--note from editor: Suggested, to fix dangling modifier.-->&mdash;the portals feature also allows users to sign in with a local contact, membership provider&ndash;based account<!--note from editor: Edit okay? There are too many nouns in a row, it's hard to see what modifies what.-->.
...
| Azure AD B2C | OpenID Connect | [Azure AD B2C with OpenID Connect](configure-azure-ad-b2c-provider.md) |
| Azure Directory Foundation Services (AD FS) | SAML 2.0 | [AD FS with SAML 2.0](configure-saml2-settings.md) |
