...
|Authentication/[Protocol]/[Provider]/AllowContactMappingWithEmail| |Allow auto-association to a contact record based on email. </br> **NOTE:** It is recommended that this is set to **False** when allowing authentication from unknown tenants. Use [invitations](invite-contacts.md) to allow users to authenticate to your portal.</br> More information: [Allow contact mapping with email and require unique email general options](use-simplified-authentication-configuration.md#general-authentication-settings). |
