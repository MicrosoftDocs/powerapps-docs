...
|Attach File |  Select to have the form include a file upload control to the bottom of the form to allow a file to be attached to the record. <br> **Note**: Portals [version 9.2.2.x and later](https://support.microsoft.com/help/4541765/power-apps-portals-version-9-2-2-x-release) do not require **Enable Entity Permissions**. If you have it selected, you must ensure appropriate privileges are provided on the parent entity and the annotation entity to display the **Attach File** button. Appointment entity must have at least **Create** and **Append** privileges and the parent entity must have the corresponding **AppendTo** privilege. Depending on whether you have a create or update form, you may also need Create, Read and Write privileges to complete the scenario of the form. Portals versions earlier than 9.2.2.x must have **Enable Entity Permissions** selected and privileges explained above configured to show **Attach File** button on the form.   |
