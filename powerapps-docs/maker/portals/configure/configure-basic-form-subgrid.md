...
If the Microsoft Dataverse form used for a basic form has an embedded subgrid, a read-only subgrid will appear on the page and display the corresponding records in a read-only list. A must have the appropriate [table permissions](assign-entity-permissions.md) in order to see the list of related records.
...
To add subgrid metadata to a basic form, go to **Basic Form Metadata** by using either the top drop-down list or the subgrid on the main form of the record that you're working with. More information: [Define basic forms](entity-forms.md).
...
:::image type="content" source="../media/add-subgrid-metadata.png" alt-text="Add subgrid metadata.":::  
...
| Access Denied Message      | Overrides the default HTML message that appears when a user doesn't have sufficient [permissions](assign-entity-permissions.md) to read the table type associated with the subgrid.                       |
