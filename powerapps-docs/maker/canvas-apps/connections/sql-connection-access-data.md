...
If you use 'Start with data' to create an app you will find that that the Items property of your gallery has a Power Fx formula in it with the simple datasource name that points directly to your database table. For example, if you have a 'BOOKLENDING' table, the formula might look like the following:

```power-fx
Search([@'[dbo].[BOOKLENDING]'], SearchInput1.Text, author, author,book_name,category)
```
In contrast, a common professional data access pattern is to use views and then stored procedures for create, update, and delete rather than allow direct access. If want to use views or stored procedures, you will need to change this formula. Similarly, the form for the record won't use the built-in direct approach of 'SubmitForm()' formula either. 

> [!NOTE]
> One database pattern is to use triggers on tables. If a table has a trigger, then you also can't use the direct pattern `Submit()` for create, update, and delete. `Submit()` has a conflict between the handling of SQL triggers and the built-in Power Apps behavior, which uses the same output parameter. You can however directly access the table for query purposes, but to handle `Create`, `Update`, or `Delete` you must call a stored procedure.
...
For example, if you had a BOOKLENDINGVIEW to your database and added it as a data source for Power Apps, the formula could be as simple as:
```power-fx
BOOKLENDINGVIEW
```
...
When you add a SQL Server connection to your app, you can add stored procedures and call them directly in Power Fx. Note that this feature also works with secure implicit connections.
