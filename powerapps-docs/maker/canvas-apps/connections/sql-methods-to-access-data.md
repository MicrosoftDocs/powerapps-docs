...
Views show up in the list of tables you can select when you add a data source. Views only support queries—not updates. You must use a stored procedure for these actions.
...
You might want the autogenerated screens from `Start with data` for views and stored procedures. For this option, choose `Start with data` with a *basic table* and then delete and replace the table data source. You can also replace other create, update, and delete formulas with a view data source and stored procedure calls.  
...
> [!IMPORTANT]
> The schema of the return values of the stored procedure should be *static*, so the values don't change from call to call. For example, if a stored procedure returns two tables, then it should *always* return two tables. You can work with either typed or untyped results.
>
