...
## Limitations
1. Note that due to the security architecture of PowerApps, PDF Viewer only supports HTTPS links and not HTTP.  

2. The Document property must link directly to the PDF file. Server redirects or HTML views of the document are not supported.

3. The server that hosts the document must not require authentication.

4. If the PDF document resides on a server with restrictive CORS settings, you may not be able to view it within your app.  To resolve this issue, the server that hosts PDF documents needs to permit cross origin requests (CORS) coming from powerapps.com.
...
If the above limitations restrict you from displaying a PDF document in an app, the recommended approach is to include the PDF file as a media resource in the app.  This will ensure the file can always be displayed in the PDF Viewer control.
