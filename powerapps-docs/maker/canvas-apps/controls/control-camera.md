...
A user can update a data source with pictures captured using camera. Images generated by the camera control aren't full resolution of the camera. If you need full resolution images, use the [Add picture](control-add-picture.md) control.
...
**Photo** – The image captured when the user takes a picture. Captured media is referenced by a URI. For more information, read the [data type documentation](../functions/data-types.md#uris-for-images-and-other-media).
...
1. Set the [Items](properties-core.md) property of the **Image gallery** control to **MyPix**.

1. Set the [Image](properties-visual.md) property of the **Image** control in the gallery to this expression:
...
1. (optional) Set the **OnSelect** property of the **Image** control in the **Image gallery** control to **Remove(MyPix, ThisItem)**, press F5, and then select a picture to remove it.
