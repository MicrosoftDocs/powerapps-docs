...
> [!NOTE]
> This article describes the updated Combo box modern control. For information about migrating from the earlier earlier version, see [Migrate to the Combo box modern control](combo-box-migration.md).
...
This updated version of the Combo box modern control includes the following improvements:

- **Multiple selection by default**: The **SelectMultiple** property now defaults to `true`, allowing users to select multiple items without additional configuration.
...
- **Immediate OnChange firing**: The **OnChange** event now fires immediately on every selection or deselection, enabling real-time dependent dropdowns and filtering. Use **DelayOutput** if you need the previous behavior.
- **DelayOutput property**: New property to control when the **OnChange** event fires—either immediately or when focus leaves the control.
- **ItemDisplayText formula**: Replaces the **Fields** property with a more flexible formula-based approach for customizing item display text.
- **Search text clears on selection**: When an item is selected, the search text automatically clears, improving the user experience.
- **Reset() includes SearchText**: The `Reset()` function now also clears the search text along with the selection.
- **TriggerOutput removed**: The **TriggerOutput** property has been removed. Use **DelayOutput** instead to control output timing.
...
- [Migrate to the Combo box modern control](combo-box-migration.md)
