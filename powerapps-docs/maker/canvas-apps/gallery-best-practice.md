...
For example, when `ComboBox.DefaultSelectedItems` changes, it will trigger `OnChange`. Consider a **Combo box** in a **Gallery** with `DefaultSelectedItems` set to `First(ThisItem.A)` and its `OnChange` property set to:
...
This causes an infinite loop. When the user changes the value of the **Combo box**, `OnChange` is triggered, which will update `GalleryData`, which will change `DefaultSelectedItems` (because `First(ThisItem.A)` refers to a new table), which will trigger `OnChange` again, and so on.
...
## Be aware that Gallery.Selected can change unexpectedly
...
This may not be desirable for your scenario. If you want a stable copy of the item that was selected by the user, consider storing it in a variable. For example, set the **OnSelect** property of the **Gallery** to a global variable `CurrentItem`:
...
The order of these events is not fixed. This is a problem if `Checkbox.OnSelect` is set to:

```powerapps-dot
Notify(Gallery.Selected.Name)
```

`Gallery.Selected` may still be referring to the previously selected row when `Checkbox.OnSelect` is executed.
...
Most of the time, the **Gallery** can guess the schema of its **Items** from the data source and expressions used in the app. But if the **Items** property is set to the output of a **Canvas component** or **[Import](controls/control-export-import.md)** control, the **Gallery** can't determine its schema. The output table from these controls may not be available when the app loads and the schema might even change. The **Gallery** will not render any items when it doesn't know its schema.
