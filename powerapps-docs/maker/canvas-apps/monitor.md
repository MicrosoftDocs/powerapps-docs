...
> [!NOTE]
> Anyone in your organization can be invited to a Monitor session. Anyone joining the session will see all the events generated by the app, including any data.

1. Select **Invite** in the top menu of the monitor.
...
You can download the events that are shown in the  table for offline analysis. Events can be downloaded in a ```.json``` or a ```.csv``` format and shared with others so they can help you with an issue. ```.csv``` files are download only, but if you export the events in ```.json``` format, these can be uploaded back into the monitor tool for analysis. You can also attach a trace file to support service requests helping expedite the solution to your problem.
