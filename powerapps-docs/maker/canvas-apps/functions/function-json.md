...
| **Currency** | Number that uses a decimal separator that reflects the user's language. Scientific notation is used if needed. | `1.345` |
...
| **Number** | Number that uses a decimal separator that's independent of the user's language. Scientific notation is used if needed. | `1.345` |
| **Option&nbsp;set** | The numeric value of the option set, not the label used for display. The numeric value is used because it's language independent.  | `1001` |
| **Time** | String that contains an ISO 8601 *hh:mm:ss.fff* format.  | `"23:12:49.000"` |
| **Record** | A comma-delimited list of fields and their values between **{** and **}**. This notation resembles that for canvas records, but the name is always double quoted. This format doesn't support records that are based on many-to-one relationships.  | `{ "First Name": "Fred", "Age": 21 }` |
| **Table** | A comma-delimited list of records between **[** and **]**. This format doesn't support tables that are based on one-to-many relationships.  | `[ { "First Name": "Fred", "Age": 21 }, { "First Name": "Jean", "Age": 20 } ]` |
| **Two&nbsp;option** | The Boolean value of the two option, *true* or *false*, not the label used for display. The Boolean value is used because it's language independent. | `false` |
| **Hyperlink, Text** | A string between double quotation marks. Embedded double-quotation marks are escaped with a backslash, newlines are replaced with "\n", and other standard JavaScript replacements are made. | `"This is a string."` | 
...
| **IncludeBinaryData** | The result includes image, video, and audio-clip columns. If you include binary data, the result's size can dramatically increase and degrade your app's performance. |
...
Use the [**ShowColumns** and **DropColumns**](function-table-shaping.md) functions to control what data the result includes and to remove unsupported data types.
...
1. Select the button while holding down the Alt key.
    ...
    The **CitiesByCountry** collection is created with this data structure, which you can show by selecting **Collections** on the **File** menu.

    ![](media/function-json/cities-grouped.png)

    You can also show this collection by selecting **File** > **App settings** > **Advanced settings**, enabling **Formula result view**, and then selecting the name of the collection in the formula bar.
