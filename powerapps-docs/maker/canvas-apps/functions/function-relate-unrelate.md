...
1. Create a new tablet app from blank.
...
1. Add a second blank vertical **Gallery** control.  Re-size the control to fill the upper right hand side of the screen. If desired, add the blue label control above the Gallery but it is not required.  This second gallery will display the reservations for the selected product in the first gallery.
...
1. Select the **Next arrow** control in the second gallery and use the formula bar to set the **OnSelect** property to **Relate( ComboBox1.Selected.Reservations, ThisItem )**.  Executing this formula will move the current reservation to a different Product.
    ![](media/function-relate-unrelate/reservations-relate.png)
1. Preview the app in the Studio.

With our app we can now move reservations from one product to another.  For a reservation on one product, you can select a different product in the combo box control and select the next arrow icon.  
...
2. Change the **OnSelect** formula to this formula:
    ```powerapps-dot
...
1. Select the second gallery (and label if you added one) and Copy to the clipboard with the Ctrl-C key.  The paste back on to the same screen with Ctrl-V key and position on the lower right of the screen.
...
With these changes, we can now clear the selection in the combo box for a reservation and disconnect it from any product.  Reservations that have no associate Product appear in the bottom gallery control, and from here they can be assigned to a product.
...
1. In the Maker portal, go to the Data > Entities page.
...
6. Save, confirm the operation, and the relationship will be added.
    ![](media/function-relate-unrelate/entity-done.png)

#### Relate and Unrelate Contacts with Products

We will create an app that is similar to the one we previously built, but will use a Many-to-Many relationship.  Instead of moving a record as we did with reservations, we will be able to relate the same record multiple times.

1. Follow steps 1-5 under One-to-Many above to add a **Gallery** control connected to the Products entity.  
...
5. Delete the **Next arrow** icon and replace it with a **Cancel** icon.  
...
1. Select **Screen1** and set the **OnVisible** property to **Refresh( Products )**.  This is important when flipping between screens since updating a One-to-Many or Many-to-Many refreshes the data only the base entity.
...
3. The name of the new screen will be **Screen1_1**.  To create the reverse view, change the following formulas:
...
4. The result will look very similar to the previous screen, but comes at the relationship from Contacts.
    ![](media/function-relate-unrelate/reverse-screen.png)
5. Insert an **Arrows up down** icon control and set its **OnSelect** property to **Navigate( Screen1, None )**.  Do the same thing to the **Screen1** with the formula **Navigate( Screen1_1, None )**.
