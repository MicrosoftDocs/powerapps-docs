...
* A custom format, which comprises a language-region indicator (en-US by default) and a string of placeholders that describe how to format the number or the date/time value. PowerApps supports a subset of the placeholders that Microsoft Excel does, and they define, for example, how many digits to show in a number and how to display the name of a month in a date/time value. The language-region indicator specifies how to interpret the placeholders. For example, a custom format might include placeholders for thousands and decimal separator, and the fr-FR indicator specifies that the decimal separator should be a comma
