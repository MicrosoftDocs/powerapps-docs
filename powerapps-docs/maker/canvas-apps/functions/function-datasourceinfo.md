...
> [!NOTE]
> **DataSourceInfo** returns *true* if it cannot determine if the current user has the requested permission.  Permissions will be checked again by the server when the actual operation is carried out and an error is displayed if it was not allowed.
