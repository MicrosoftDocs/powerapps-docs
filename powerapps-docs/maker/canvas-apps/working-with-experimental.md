...
In the final transition to **Shipped** we may turn the feature on permanently for apps in which it's already turned on by removing the Preview switch.  This will be the case for most apps as the feature will have been on by default before getting to this stage.  For apps where the feature is turned off, the Preview switch will still be available for you to turn on, test with the feature, and still turn off within the same Studio session.  But if the switch is turned on when the app is saved, the switch will not be available when the app is reloaded and the feature can not be turned off again. At that point you can [restore the app to a previous version](restore-an-app.md) to bring the app back to a state before the feature was turned on.
