...
Some connections (such as to a SQL Server with SQL or Windows authentication) are automatically shared with the app when you share the app with other users. Other connections require users to create their own connections (such as OneDrive for Business or SQL Server with Azure AD authentication).
