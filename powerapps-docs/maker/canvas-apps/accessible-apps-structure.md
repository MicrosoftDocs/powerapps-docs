...
> - When a control's **X** or **Y** value is set to an expression, the control order is not updated when the expression changes. The order is fixed when the app is saved. Expressions will be evaluated based on the initial state of the app.
