...
1. In the Choose a template screen, select **Empty job**:

    ![Empty job](media/test-studio-classic-pipeline-editor/empty-job.png)
...
1. Select **+** next to the *Agent job 1*.
...
1. Select the task. <br>
    You can also update display name to *Set Agent Screen
    Resolution to 1920x1080* or similar.

1. Select **Inline** as the script type, and enter the below in the script window:
...
1. Select **+** beside the Agent job 1.
...
1. Select **OK**:
    ...
    ![NuGet package](media/test-studio-classic-pipeline-editor/nuget.png)
...
1. Expand the **Microsoft.PowerApps.TestAutomation.Tests**, select the
    **patestautomation.runsettings** file, and then select **OK**:
    ...
    ![Run settings](media/test-studio-classic-pipeline-editor/runsettings.png)
...
1. Enter **Run Power Apps Test Automation Tests via \$(BrowserTypeChrome)** or similar in the Test Run Title field.
    ...
    ![Test run](media/test-studio-classic-pipeline-editor/test-run.png)
...
You'll now configure the pipeline variables defined in the tasks
you've added [earlier](#add-tasks-to-the-pipeline).
