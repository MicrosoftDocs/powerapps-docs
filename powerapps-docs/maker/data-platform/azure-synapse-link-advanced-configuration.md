...
| Data integration | Dataverse tables and Finance and Operations tables and Entities | "Append only option" provides CSV files that contain incremental data. You can build pipelines that consume incremental data and populate downstream systems <br> "User-specified data partition" feature enables choosing a custom data partitioning strategy specifically for Dataverse Tables. This option is not available for Finance and Operations Apps|     
