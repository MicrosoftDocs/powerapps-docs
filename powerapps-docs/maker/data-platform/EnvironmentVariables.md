...
- Environment variables for SharePoint must have matching metadata. SharePoint has internal identifiers that may not match between target envronments. For example, if you simply create a list with the same name and columns in a target environment, the internal names will likely not match.  The metadata will always match if you duplicate a SharePoint site and copy it to your target environment. 
