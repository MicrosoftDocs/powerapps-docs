...
| Metadata field  |  How to use this field | equivalent field in Export to Data lake |
|-------------------------------|------------------------------------|------------------------------------------|
...
| IsDelete | if True this record is deleted from Dataverse or FnO | Export to Data lake deletes the row from the destination immediately. Synapse link performs a "soft delete" for delta format such that you can identify deleted rows without consuming change feeds. <br><br>In case of Delta files, soft deleted rows are purged after 28 days.<br><br>IsDelete field is also available in incremental updates. In case you want to access latest row for incremental update isDelete is False, latest version number and sync modified on date for a given ID  |
| modifiedon |  Indicates the date and time the record was last modified | This field is populated from FnO modified date time |
| modifiedtransactionid |  internal use | used internally - you should not use this field |
| SinkCreatedOn | indicates the date the record was written to the Data lake | In case of incremental updates, data and time data was written to lake is shown. For Delta format updates, indicates the date and time of delta lake conversion |
| SinkModifiedOn |  | incremental - last created/modified for Delta same as SinkCreatedOn  -- check with Deepak |
| sysdatastatecode | if 1, this record is archived using the Long term retention feature. If 0 this is a live record | You can use this field to identify FnO rows that have been archived (and deleted form live data). The same field is available for for incremental updates |
| sysrowversion | used internally to determine incremental or delta changed to be processed | This is an internal field not available for customers | 
| tableid |  Contains a unique ID of each table | Contains the Table ID from finance and operations |
| versionnumber | last version of the row that has been synced to Data lake | This is an internal field not available for customers  |

## Reacting to system events and generating audit reports 
If you used [business events generated by Export to Data lake feature](https://learn.microsoft.com/dynamics365/fin-ops-core/dev-itpro/data-entities/azure-data-lake-generates-biz-events) in finance and operations to react to system events, you can continue to get alerted via Synapse Link service. In addition, you can generate audit reports and Power Apps to monitor specific data of interest with Synapse Link configuration data saved as Dataverse tables. For more information on receiving and consuming events from Synapse Link [see the documents here](https://learn.microsoft.com/power-apps/maker/data-platform/azure-synapse-link-notifications). Folowing table summarizes some changes. 

| Export to Data lake event  |  ... indicates  | How you can use configuration data in Synapse Link |
|----------------------------|----------------------------|------------------------------------------|
...
| **Metadata changed** | This event indicates that table metadata has changed. For example, a new column has been added to the table, or (in rarer cases) a column has been deleted. You'll receive multiple events for each table, one for each metadata change. If you have downstream data pipelines that can't react to metadata changes (a situation that is known as schema drift), you can detect changes by using these events and modify downstream pipelines | **Last Synced Metadata Time** field in **Synapse Link Profile Entity State** can be used to detect metadata changes |
| **Deactivated**  | This event indicates that a previously active table has been inactivated. When a table is inactivated, data that is changed in finance and operations apps isn't updated in the data lake | hard delete in the **Synapse Link Profile Entity State** table | 
| Activated |  This event indicates that a previously inactivated table has been reactivated. When a table is reactivated, the system reinitializes data in the data lake. You might later receive an Initialize complete event | new record in the hard delete in the **Synapse Link Profile Entity State** table |
| Error | This event indicates that the Export to Data Lake service has paused updates to data in the data lake. | **Synapse Link Profile Entity State** status field |
| Lake switch | This event indicates that the system administrator has changed the location of the data lake | Changing a Data lake associated with a Synapse Link profile is not supported. Synapse Link supports multiple profiles - this means, an administrator can   create multiple Azure Synapse Link profiles with tables (the same table can be added to multiple profiles). You can read the Dataverse table **Synapse Link profile** to read configuration data on all profiles including deleted profiles |  
