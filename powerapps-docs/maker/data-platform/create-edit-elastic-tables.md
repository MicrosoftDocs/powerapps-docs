...
- When TTL is used on a row, the row will get deleted from Elastic table when TTL has expired but if its synced to data lake via “Synapse link for dataverse” before expiry, it won’t be deleted from data lake
