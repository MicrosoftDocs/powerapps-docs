...
Columns might have different types of data such as numerals, strings, digital data, images, and files. There is no need to keep relational and non-relational data separated artificially if it is part of the same business process or flow. Dataverse stores the data in the best storage type for the model created.
