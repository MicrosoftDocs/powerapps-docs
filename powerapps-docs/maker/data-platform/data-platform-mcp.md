...
- By default, the Dataverse MCP server is disabled for environments. You must enable it in the Power Platform admin center before you can connect to it. More information: [Enable the Dataverse MCP server using advanced connector policies (preview)](data-platform-mcp-disable.md#enable-the-dataverse-mcp-server-using-advanced-connector-policies-preview)
