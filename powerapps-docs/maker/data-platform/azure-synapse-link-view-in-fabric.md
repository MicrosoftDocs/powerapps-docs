...
| Error message "Unauthorized. Access to target location https://[...].crm3.dynamics.com/ denied" when accessing Dataverse tables in Fabric Lakehouse | This error message indicates that the user accessing tables in Fabric doesn't have the required permissions to access Datavese tables. This error may be shown even if the same user has access to the tables in PowerApps. <br> Link to Fabric wizard in PowerApps maker portal creates a data connection at the time of creating the Link to Fabric. Ensure that: <br> - The password of the user who created the connection has not expired <br> - the user account of the user who created Fabric Link is active <br> - other users who need to access Dataverse tables in Fabric Lakehouse need to be granted access to the data connection. See [Secure data and grant access to other users](#Secure data and grant access to other users) for more information |
