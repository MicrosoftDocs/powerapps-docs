...
Admins can manage tables linked to OneLake from the **Azure Synapse Link for Dataverse** page. If this environment is linked to Fabric, you will see a link called **Microsoft OneLake**.
...
You must be a Systems Administraror in the PowerApps enviromment to enable Link to Fabric. You must also have Owner access to the Fabric workspace linked with Dataverse. 

You need to grant access to other users to this workspace so that they can work with data. Depending on the need for data access, you may need to secure the data in this workspace before you share this data with others. More information: [OneLake security guidelines](/fabric/onelake/get-started-security)
