...
The system creates a data connection between Power Apps environment and Fabric workspace using the credentials of the user at the time of creating the Link. If you use Fabric Link option from Power Apps tables menu, the system creates the connection and asks you to save. If you use Synapse Link option, you will need to create a data connection yourself.

System uses this connection to enable Fabric users to connect to Dataverse - the data store behing Power Apps environment. If you need to enable other users to consume this data in Fabric Lakehouse, you need to share this data connection with other users. 

You need to grant access to other users to this workspace so that they can work with data. Depending on the need for data access, you might need to secure the data in this workspace before you share this data with others. More information: [OneLake security overview](/fabric/onelake/security/get-started-security)
