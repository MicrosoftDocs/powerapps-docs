...
- A Power BI premium license or Fabric capacity within the same Azure geo as your Dataverse environment is required. Currently, the system supports these premium capacity SKUs: "P1", "P2", "P3", "P4", "P5", "F2", "F4", "F8", "F16", "F32", "F64", "F128", "F256", "F512", "F512", "F1024", "F2048", "DCT1", "FT1."
...
You must have the Systems Administrator security role in the Power Platform environment to enable **Link to Fabric** or **Synapse Link**. You must must be an administrator of the Power BI workspace. If you want the system to create a Power BI workspace, you need to be a Power BI Capacity Administrator or have contributor access to a capacity within the same region as the Dataverse environment.

The system creates a data connection between Power Apps environment and Fabric workspace using the credentials of the user at the time of creating the Link. If you use Fabric Link option from Power Apps tables menu, the system creates the connection and asks you to save. If you use Synapse Link option, you will need to create a data connection yourself before enabling the link.

System uses this connection to enable Fabric users to connect to Dataverse - the data store behing Power Apps environment. If you want to enable other users to consume this data in Fabric Lakehouse, you need to share this data connection with other users. To share the data connection with other users, 
1. Launch Fabric.Microsoft.com and select the gear icon on top left (next to user icon)
2. In the **Settings** menu, select **Data connections and Gateways** option. Available data connections will be shown
3. Select **Connections** tab and Choose the data connection you created for Connection type **Dataverse**. You may see a connection that is named like **org...crm.dynamics.com**. In case you have multiple connections like this, you will need to select the connection that links to the specific Power Apps environment.
4. Once you seleted the correct data connection, select the **...** and choose **manage users** option. You will be shown users who have access to this connection
5. You can enter the name or email of other users who need access to data. When you select a user, you can specify either **Owner** role or **Reader** role. You only need to provide reader role to enable them to consume data.
6. They will receive an e-mail confirming access to data. 

In addition to granting them with access to data connection, you need to grant access to other users to this workspace so that they can work with data. Depending on the need for data access, you might need to secure the data in this workspace before you share this data with others. More information: [OneLake security overview](/fabric/onelake/security/get-started-security)
...
| Newly added fields in tables are not reflected in Fabric Lakehouse | When a new field is added to a Dataverse table, the newly added field is not added to Fabric Linked workspaces automatically. To include newly added fields, visit Synapse Link menu in PowerApps maker portal, choose the Microsoft OneLake or the Synapse Link profile and select **Refresh Fabric tables** options. This will refresh the table metadata in Fabric Lakehouse |
| Error message "Unauthorized. Access to target location https://[...].crm3.dynamics.com/ denied" when accessing Dataverse tables in Fabric Lakehouse | This error message indicates that the user accessing tables in Fabric doesn't have the required permissions to access Datavese tables. This error may be shown even if the same user has access to the tables in PowerApps. <br> Link to Fabric wizard in PowerApps maker portal creates a data connection at the time of creating the Link to Fabric. This data connection uses credentials of the user, it is possible that: <br> - The password of the user who created the connection has changed or has expired <br> - the user account of the user who created Fabric Link is inactive <br> - other users who need to access Dataverse tables in Fabric Lakehouse do not have access to the data connection. See [Secure data and grant access to other users](#Secure data and grant access to other users) for more information |
...
| Tables from Dynamics 365 Finance and Operations are missing | System auto selects non-system tables with **Change tracking** property set to **Yes** when creating a Fabric Link. To select more tables, you can open **Microsoft OneLake** profile in Synapse Link menu and choose **Manage Tables** option. If you have a Finance and Operations environment linked to this Power Apps envionment, you can also select tables from Finance and Operations. Finance and Operations tables are not auto-selected with Fabric link. <br> For more details on selecting Finance and Operations tables in Synapse Link see [Add Finance and Operations tables](https://learn.microsoft.com/power-apps/maker/data-platform/azure-synapse-link-select-fno-data#add-finance-and-operations-tables-in-azure-synapse-link) |
| There are {.. #tables} tables enabled for change tracking in your environment. Addition of more than thousand tables is not supported. | Fabric Link feature can't be enabled on environments with more than 1,000 change tracking enabled tables. We are working on increasing this limit in the coming months. <br> As a workaround, you can use Synapse Link option and create multiple profiles with less than 1,000 tables each |     
