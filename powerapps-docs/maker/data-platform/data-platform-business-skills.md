...
There are two ways to create business skills: 

- Use the Dataverse MCP server (preview) to create skills. More information: [Configure the Dataverse MCP server for an environment](data-platform-mcp-disable.md)
- The Dataverse MCP server (preview) has new tools to create skills. To create  a business skill using the tools:

   1. Add Dataverse MCP server Preview as a tool to any agent in Microsoft Copilot Studio or connect to it from coding agents in Visual Studio Code/ non-microsoft clients.
   1. Start by asking your agent to create a new skill and define its purpose.
   1. Provide your business process information.  
   1. The business skill will be saved in the environment you used to connect to the MCP server.  

Manually create skills in Power Apps maker portal:

1. Go to [Power Apps](https://make.powerapps.com) and then go to **Tables**.  
1. Select **All** in the **Tables** list and search for **Business Skills**. This table is where all your skills are stored in your environment.  
...
You can now use this app to manage and create skills manually going forward.

> [!NOTE]
> - Notice that this app also appears with all your other apps in the environment.  
> - All skills are active by default. [Deactivate a skill](#deactivate-a-skill)

### Deactivate a skill

To prevent a skill from being used in agents until you have finalized them, select **Deactivate** on the command bar. Deactivate doesn't delete the skill, but prevents agents to see it until you activate them.
