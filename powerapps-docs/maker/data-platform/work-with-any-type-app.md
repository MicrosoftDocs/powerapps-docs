...
The API is secured by using OAuth. OAuth requires an `idtable` provider for authentication. For Dataverse, the `idtable` provider is Microsoft Entra ID. To authenticate with Microsoft Entra by using a Microsoft work or school account, use the Microsoft Authentication Libraries (MSAL).
