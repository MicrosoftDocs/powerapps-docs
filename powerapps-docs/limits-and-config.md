...
## Supported platforms for running apps by using the Power Apps mobile app

> [!NOTE]
> - To ensure an optimal user experience, test your app on the specific devices you plan to use for production deployment. 
> - Performance testing and results can vary between device types such as mobile, desktop, and laptops. Factors include processing power, memory capacity, network connectivity, app complexity, and other apps running in parallel on a device. Make sure the test results meet your business requirements before you roll out the solutions in production.
...
> [!NOTE]
> New major versions of iOS and Android release each year. When a new version releases, if you're using the oldest previously supported version, you have 60 days to update your device to at least the new minimum supported version to continue to run Power Apps mobile.
...
For classic web application system requirements, see [Web application requirements](/power-platform/admin/web-application-requirements).
...
> [!NOTE]
> The retry value might vary. For certain error conditions, retrying isn't necessary.

## IP addresses

Requests from Power Apps use IP addresses that depend on the region of the [environment](/power-platform/admin/environments-overview) that the app is in. Microsoft doesn't publish fully qualified domain names for Power Apps scenarios.
...
| \*.azureedge.net |https | create.powerapps.com, content.powerapps.com, and make.powerapps.com <br>(Optional) We highly recommend that you use the wildcard listed under Domains. You can [download](https://go.microsoft.com/fwlink/?linkid=2225562) the complete list if you want to allow specific domain names instead of using *.azureedge.net. However, this list is subject to change.</br> |
...
| \*.blob.core.windows.net |https | Blob storage <br>(Optional) We highly recommend that you use the wildcard listed under Domains. You can [download](https://go.microsoft.com/fwlink/?linkid=2225562) the complete list if you want to allow specific domain names instead of using *.blob.core.windows.net. However, this list is subject to change.</br> |
...
> - If you're a US Government customer or a customer in China, unblock additional service URLs. For more information:<br/>**US Government**: [Power Apps US Government service URLs](/power-platform/admin/powerapps-us-government#power-apps-us-government-service-urls), [Power Automate US Government service URLs](/power-automate/us-govt#power-automate-us-government-service-urls)<br/>**China**: [Power Platform and Dynamics 365 apps - operated by 21Vianet in China](/power-platform/admin/about-microsoft-cloud-china).

## Deprecated endpoints

This section lists endpoints that Microsoft no longer supports. Instead, use the new endpoints listed in the table. Update any bookmarks that use the deprecated endpoint to the new endpoint.
...
Power Apps doesn't support running with a proxy enabled. This limitation can cause unpredictable behavior. If you encounter problems, disable the proxy and then try again.

- Some proxies, such as Zscaler and Blue Coat, remove headers from Power Apps requests. Power Apps relies on these headers, like CORS or authentication headers, to load the app.
- Some proxies, such as Microsoft Defender for Cloud Apps and McAfee, might intercept and change the URL of an app or embedded app. For example, if there's a Dynamics 365 app that runs under domain **org.crm.dynamics.com** or a canvas app that runs under domain **apps.powerapps.com**, the platform doesn't support a proxy that changes these domains to a custom domain such as **mycustomdomain.com**. This limitation can cause unpredictable behavior when the platform tries to retrieve tokens that are necessary to run the app.

## Local network access and permission problems
...
For more information, see [Power Apps per app plan](/power-platform/admin/signup-for-powerapps-admin#power-apps-per-app-plan) in the Power Platform admin guide.
